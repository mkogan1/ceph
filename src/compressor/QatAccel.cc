...
bool QatAccel::init(const std::string &alg) {
  ...
  if ( QZ_DEFLATE_GZIP_EXT == data_format(g_ceph_context->_conf.get_val<std::string>("qat_compressor_data_fmt"))) {
    windowBits = GZIP_WRAPPER + MAX_WBITS;
    dout(15) << "QAT Compressor Data format: QZ_DEFLATE_GZIP_EXT" << dendl;
  } else {
    windowBits = -MAX_WBITS;
    dout(15) << "QAT Compressor Data format: QZ_DEFLATE_RAW" << dendl;
  }
...
int QatAccel::compress(const bufferlist &in, bufferlist &out, std::optional<int32_t> &compressor_message) {
  ...
  std::clog << "// OK " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): QAT Compress" << std::endl;
...
int QatAccel::decompress(bufferlist::const_iterator &p,
  ...
  std::clog << "// OK " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): QAT DeCompress" << std::endl;
  auto s = get_session(); // get a session from the pool
  if (!s) {
    return -1; // session initialization failed
  }
