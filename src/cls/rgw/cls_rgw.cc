                              ...
                              void *param)
{
  ...
  if (iter == keys.end()) {
    if (truncated && *truncated) {
      // in the upstream version this is an assert, but since this
      // issue will not lead to data corruption, it's safer in the
      // downstream fix simply to log the issue
      CLS_LOG(0, "%s LOGIC ERROR: if no entries truncated must be false\n",
	      __func__);
      *truncated = false;
    }
    return 0;
  ...
  if (!truncated || *truncated) {
    if (!last_key) {
      ...
      // issue will not lead to data corruption, it's safer in the
      // downstream fix simply to log the issue
      CLS_LOG(0, "%s LOGIC ERROR: last_key not set\n", __func__);
    } else {
      out_marker = *last_key;
    }
  }
