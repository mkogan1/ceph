...
std::vector<Option> get_global_options() {
  return std::vector<Option>({
    ...
    .set_default("cephfs hello journal lock log numops " "otp rbd refcount rgw rgw_gc timeindex user version cas cmpomap 2pc_queue fifo")
    .set_description(""),
...
std::vector<Option> get_rgw_options() {
  return std::vector<Option>({
    ...
    .set_long_description(
      ...
      "data changes log to peer zones, so they can prioritize sync of some of "
      "the most recent changes. Can be disabled with 0."),
