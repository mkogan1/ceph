...
bool MonmapMonitor::prepare_command(MonOpRequestRef op)
{
  ...
  if (prefix == "mon add") {
    ...
    cmd_getval(g_ceph_context, cmdmap, "location", locationvec);
	...
	goto reply;
      ...
      if (!cmd_getval(g_ceph_context, cmdmap, "tiebreaker_mon", tiebreaker_mon)) {
	ss << "must specify a tiebreaker monitor";
	err = -EINVAL;
	goto reply;
      }
      string new_crush_rule;
      if (!cmd_getval(g_ceph_context, cmdmap, "new_crush_rule", new_crush_rule)) {
	ss << "must specify a new crush rule that spreads out copies over multiple sites";
	err = -EINVAL;
	goto reply;
      }
      string dividing_bucket;
      if (!cmd_getval(g_ceph_context, cmdmap, "dividing_bucket", dividing_bucket)) {
