...
class OrchestratorCli(OrchestratorClientMixin, MgrModule,
    ...
    @_cli_write_command('orch prometheus set-remote-write')
    ...
    @_cli_write_command('orch prometheus remove-remote-write')
...
"""
  ...
  ceph orch tuned-profile apply <profile_name> [--placement=<placement_string>] [--settings='option=value,option2=value2']
    ...
    @_cli_write_command('orch accept-license')
    def _accept_ibm_license(self, image: str) -> HandleCommandResult:
        """
        Fetch and display the IBM license for a given container image
        """
        completion = self.accept_license(image)
        raise_if_exception(completion)
        return HandleCommandResult(stdout=completion.result_str())
    ...
    @_cli_write_command('orch deny call-home-enabled')
