                                              ...
                                              RGWUserInfo& user_info) const      /* out */
{
  ...
  int ret = rgw_store_user_info(store, user_info, nullptr, nullptr, real_time(), true);
  if (ret < 0) {
...
void rgw::auth::WebIdentityApplier::load_acct_info(const DoutPrefixProvider* dpp, RGWUserInfo& user_info) const {
  ...
  if (rgw_get_user_info_by_uid(store, federated_user, user_info) >= 0) {
    /* Succeeded. */
    return;
  }
  ...
  int ret = store->get_user_stats(federated_user, stats);
  if (ret < 0 && ret != -ENOENT) {
