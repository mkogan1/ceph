...
{
  std::clog << "// >>>> ERROR " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): rgw::sal::Attrs attrs; // ignored --> info.user_id=" << info.user_id << std::endl;  // << user->get_id()
                           ...
                           req_state* const s, optional_yield y) noexcept
{
  try {
    ...
    try {
      ...
      std::clog << "    >>> OK " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): applier->load_acct_info() >>  s->user=" << s->user << ", s->bucket_owner.id=" << s->bucket_owner.id << std::endl;
      applier->load_acct_info(dpp, &s->user);
      std::clog << "    <<< OK " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): applier->load_acct_info() <<  s->user=" << s->user << ", s->bucket_owner.id=" << s->bucket_owner.id << std::endl;
      //MK// vvv TBD - neded ?
      s->perm_mask = applier->get_perm_mask();  //MK-TODO////MK-!!!//
...
ACLOwner rgw::auth::LocalApplier::get_aclowner() const
{
  ...
  if (account) {
    ...
    std::clog << "!!!!!=====>>>>> OK " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): owner.id=" << owner.id << std::endl;
...
void rgw::auth::LocalApplier::load_acct_info(const DoutPrefixProvider* dpp, std::unique_ptr<rgw::sal::User>* puser) const /* out */
{
  ...
  // user->get()->get_info() = this->user->get()->get_info();
  // puser = this->user;
  std::clog << "      >>> WARN puser->swap() " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): *this->user=" << *this->muser << std::endl;
  // puser->reset(this->user->release());
  // puser->reset(this->user.release());
  // puser->reset(&(*this->user));
  // puser->reset(user);
  // *puser = std::move(this->user);
  // *puser = this->user;
  // puser = this->user;
  // puser->swap((this->user));
  
  // copy the value of user to user_copy
  // user_copy = std::make_unique<rgw::sal::User>(*user);
  // user_copy = *(user.get());
  // puser.swap(this->user_copy);
  puser->swap(this->muser);
  std::clog << "      <<< WARN puser->swap() " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): *puser=" << *puser << std::endl;
}
