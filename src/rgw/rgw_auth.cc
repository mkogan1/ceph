...
auto rgw::auth::WebIdentityApplier::load_acct_info(const DoutPrefixProvider* dpp) -> std::unique_ptr<rgw::sal::User> {
...
auto rgw::auth::RemoteApplier::load_acct_info(const DoutPrefixProvider* dpp)  -> std::unique_ptr<rgw::sal::User>      /* out */
{
  ...
  if (split_mode && !implicit_tenant)
    ...
    if (user->load_user(dpp, null_yield) >= 0) {
      ...
      //MK// puser->get()->get_info() = std::move(user->get_info());
...
auto rgw::auth::LocalApplier::load_acct_info(const DoutPrefixProvider* dpp) -> std::unique_ptr<rgw::sal::User> /* out */
{
...
rgw::auth::LocalApplier::LocalApplier(CephContext* const cct,
  ...
  : user_info(puser->get()->get_info()),
    muser(std::move(*puser)),
    account(std::move(account)),
    policies(std::move(policies)),
...
auto rgw::auth::RoleApplier::load_acct_info(const DoutPrefixProvider* dpp) -> std::unique_ptr<rgw::sal::User> /* out */
{
  /* Load the user id */
  //MK// puser->get()->get_info().user_id = this->token_attrs.user_id;
  //MK-XXX//
  // std::unique_ptr<rgw::sal::User> user = driver->get_user(federated_user);
  std::clog << "// ERROR " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "():" << std::endl;
}
