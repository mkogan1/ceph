...
template <typename T>
auto ThirdPartyAccountApplier<T>::load_acct_info(const DoutPrefixProvider* dpp) -> std::unique_ptr<rgw::sal::User>
{
  std::unique_ptr<rgw::sal::User> userl;
  if (UNKNOWN_ACCT == acct_user_override) {
    ...
    userl = DecoratedApplier<T>::load_acct_info(dpp);  //MK-XXX//
    ...
    //MK// std::unique_ptr<rgw::sal::User> user; // = driver->get_user(RGW_USER_ANON_ID);  //MK// [01]-->
    if (acct_user_override.tenant.empty())
      userl->get_info().user_id = rgw_user(acct_user_override.id, RGW_USER_ANON_ID);
    else
      userl->get_info().user_id = rgw_user(acct_user_override.tenant, RGW_USER_ANON_ID);
  } else {
    ...
    if (acct_user_override.tenant.empty()) {
      ...
      if (user->load_user(dpp, null_yield) >= 0) {
        userl->get_info() = user->get_info();  //MK-XXX//
        /* Succeeded. */
        return userl;
      }
    }
...
template <typename T>
auto SysReqApplier<T>::load_acct_info(const DoutPrefixProvider* dpp) -> std::unique_ptr<rgw::sal::User>
{
  std::clog << "    //>> WARN " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): DecoratedApplier<T>::load_acct_info(dpp);" << std::endl;
  std::unique_ptr<rgw::sal::User> user = DecoratedApplier<T>::load_acct_info(dpp);
  std::clog << "    //<< WARN " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): DecoratedApplier<T>::load_acct_info(dpp); : user=" << user << std::endl;
  ...
  if (is_system) {
    ...
    if (!str.empty()) {
      ...
      if (const auto* uid = std::get_if<rgw_user>(&effective_owner->id); uid) {
        //MK// std::unique_ptr<rgw::sal::User> user = driver->get_user(*uid);
  ...
  // return nullptr;
  std::clog << "    // ERROR " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): return ~~nullptr~~ --> user" << std::endl;
  return user;
}

template <typename T>
void SysReqApplier<T>::modify_request_state(const DoutPrefixProvider* dpp, req_state* const s) const  //MK-XXX// const
{
  if (boost::logic::indeterminate(is_system)) {
    //MK// load_acct_info(dpp, &unused_user);

    // const std::unique_ptr<rgw::sal::User> unused_user {load_acct_info(dpp)};
    // error: 'this' argument to member function 'load_acct_info' has type 'const rgw::auth::SysReqApplier<rgw::auth::RemoteApplier>', but function is not marked const

    // std::unique_ptr<rgw::sal::User> unused_user = s->user->clone();
    // unused_user = load_acct_info(dpp); //MK-XXX//
    // error: 'this' argument to member function 'load_acct_info' has type 'const rgw::auth::SysReqApplier<rgw::auth::RemoteApplier>', but function is not marked const
  }
