...
{
  //MK-vvv//
  std::clog << "== CHECK >>" << __FILE__ << " #" << __LINE__ << " | " << __func__ << "():" << std::endl;
  std::unique_ptr<rgw::sal::User> luser;
  // std::unique_ptr<rgw::sal::User> luser = s->user.clone();
  // std::unique_ptr<rgw::sal::User> luser = std::make_unique<rgw::sal::User>();
  if (UNKNOWN_ACCT == acct_user_override) {
    ...
    luser = DecoratedApplier<T>::load_acct_info(dpp)->clone();
    //MK-XXX//
    ...
    if (acct_user_override.tenant.empty()) {
      ...
      if (user->load_user(dpp, null_yield) >= 0) {
        luser->get_info() = user->get_info();
        //MK-XXX//
        /* Succeeded. */
        std::clog << "== CHECK <<" << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): luser=" << luser << std::endl;
        return luser;
      }
    }
...
void SysReqApplier<T>::modify_request_state(const DoutPrefixProvider* dpp, req_state* const s) const
{
  if (boost::logic::indeterminate(is_system)) {
    ...
    // unused_user = load_acct_info(dpp)->clone(); //MK-XXX//
    // error: 'this' argument to member function 'load_acct_info' has type 'const rgw::auth::SysReqApplier<rgw::auth::RemoteApplier>', but function is not marked const
  }
