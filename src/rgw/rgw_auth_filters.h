...
void SysReqApplier<T>::load_acct_info(const DoutPrefixProvider* dpp, std::unique_ptr<rgw::sal::User>* puser) const
{
  ...
  std::clog << "// xxxx " << __FILE__ << " #" << __LINE__ << " | " << __func__ << "(): *puser=" << *puser << ", is_system=" << is_system.value << std::endl;
  if (is_system) {
    ...
    if (!str.empty()) {
      ...
      if (const auto* uid = std::get_if<rgw_user>(&effective_owner->id); uid) {
        ...
        // user->read_attrs(dpp, null_yield);
      }
    }
  } else {
    ...
    // try {
    //   int ret = user->read_attrs(dpp, null_yield);
    //   if (ret != 0) {
    //     ldpp_dout(dpp, 0) << "couldn't get user attrs: user_id=" << user->get_id() << ", ret=" << ret << dendl;
    //   }
...
void SysReqApplier<T>::modify_request_state(const DoutPrefixProvider* dpp, req_state* const s) const
{
  if (boost::logic::indeterminate(is_system)) {
    ...
    std::unique_ptr<rgw::sal::User> unused_user;  //MK// //MK-TODO// //!!!//
