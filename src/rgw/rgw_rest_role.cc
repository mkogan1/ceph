...
void RGWUpdateRole::execute()
{
  ...
  if (!store->svc.zone->is_meta_master()) {
    ...
    RGWUserInfo info;
    op_ret = rgw_get_user_info_by_uid(store, s->user->user_id, info);
    if (!op_ret) {
      return;
    }
...
#if 0
    op_ret = store->forward_iam_request_to_master(s, key, nullptr, post_body, &parser, s->info, y);
    if (op_ret < 0) {
