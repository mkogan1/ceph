					...
					int64_t end, RGWGetDataCB* cb, optional_yield y)
{
  ...
  if (posix_try_use_uring(dpp, y, /*nsfs=*/true)) {
    ...
    if (fd >= 0) {
      ...
      if (r < 0) {
        ldpp_dout(dpp, 1) << "URING: NSFSReadOp::iterate failed: "
...
int NSFSMultipartWriter::process(bufferlist&& data, uint64_t offset)
{
  if (posix_try_use_uring(dpp, yield, /*nsfs=*/true) && part_file &&
    ...
    if (r < 0) {
      ldpp_dout(dpp, 1) << "URING: NSFSMultipartWriter::process failed: "
                        << cpp_strerror(-r) << " (" << r << ")" << dendl;
    }
    return r;
  }
                       ...
                       uint32_t flags)
{
  ...
  if (uring_write) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "URING: NSFSMultipartWriter::complete drain failed: "
                        << cpp_strerror(-ret) << " (" << ret << ")" << dendl;
      return ret;
    }
  } else {
...
int NSFSAtomicWriter::process(bufferlist&& data, uint64_t offset)
{
  ...
  if (posix_try_use_uring(dpp, yield, /*nsfs=*/true) && fd >= 0) {
    ...
    int r = uring_write->process(std::move(data), offset);
    if (r < 0) {
      ldpp_dout(dpp, 1) << "URING: NSFSAtomicWriter::process failed: "
                        << cpp_strerror(-r) << " (" << r << ")" << dendl;
    }
    return r;
  }
                       ...
                       uint32_t flags)
{
  ...
  if (uring_write) {
    ret = uring_write->drain();
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "URING: NSFSAtomicWriter::complete drain failed: "
                        << cpp_strerror(-ret) << " (" << ret << ")" << dendl;
      return ret;
    }
  } else {
