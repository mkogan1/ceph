			     ...
			     rgw::sal::RadosStore* _store)
{
  ...
  sys::error_code ec;
  loc = rgw::init_iocontext(dpp, store->get_neorados(), log_pool, true, false,
			    async::use_blocked[ec]);
  if (ec) {
    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__
		       << ": Failed to initialized ioctx, ec=" << ec.message()
		       << ", pool=" << log_pool << dendl;
				 ...
				 const rgw_bucket& bucket) const
{
  ...
  co_return co_await
    async::async_yield<yield_context>(
      store->get_io_context().get_executor(),
      [this, dpp, &bucket](yield_context yc) {
