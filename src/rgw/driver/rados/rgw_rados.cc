                                           ...
                                           map<string, bufferlist>& attrs, optional_yield y)
{
  ...
  int r;
  if (assume_noent) {
    r = _do_write_meta(dpp, size, accounted_size, attrs, assume_noent, (void *)&index_op, y);
    if (r == -EEXIST) {
      assume_noent = false;
    }
  }
               ...
               std::optional<uint64_t>* bytes_transferred)
{
  ...
  AtomicObjectProcessor processor(&aio, this, dest_bucket_info, nullptr,
                                  user_id, dest_obj_ctx, dest_obj, olh_epoch,
				  tag, dpp, null_yield);
  RGWRESTConn *conn;
  auto& zone_conn_map = svc.zone->get_zone_conn_map();
  auto& zonegroup_conn_map = svc.zone->get_zonegroup_conn_map();
  ...
  if (copy_if_newer) {
    ...
    ret = get_obj_state(dpp, &dest_obj_ctx, dest_bucket_info, stat_dest_obj, &dest_state, &manifest, stat_follow_olh, null_yield);
    if (ret < 0)
      goto set_err_state;
...
set_err_state:
  if (copy_if_newer && ret == -ERR_NOT_MODIFIED) {
    ...
    if (olh_epoch && *olh_epoch > 0) {
      ...
      ret = set_olh(dpp, dest_obj_ctx, dest_bucket_info, dest_obj, false, nullptr,
               ...
               optional_yield y)
{
  ...
  if (remote_src || !source_zone.empty()) {
    ...
    return fetch_remote_obj(dest_obj_ctx, user_id, info, source_zone,
               ...
               nullptr /* filter */, source_trace_entry, stat_follow_olh, stat_dest_obj);
  }
  ...
  if (copy_data) { /* refcounting tail wouldn't work here, just copy the data */
    ...
    return copy_obj_data(dest_obj_ctx, dest_bucket_info, dest_placement, read_op, obj_size - 1, dest_obj,
                         mtime, real_time(), attrs, olh_epoch, delete_at, petag, read_filter, dpp, y);
  }
 ...
 * Returns: 0 on success, -ERR# otherwise.
 */
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider *dpp, const bool force)
{
  RGWRados *store = target->get_store();
  ...
  if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
	...
	store->gen_rand_obj_instance_name(&marker);
      ...
      r = store->unlink_obj_instance(dpp, target->get_ctx(), target->get_bucket_info(), obj, params.olh_epoch,
                                     y, params.bilog_flags, params.null_verid, params.zones_trace, force);
      if (r < 0) {
        return r;
      }
  ...
  if (r >= 0 || force) {
    ...
    r = index_op.complete_del(dpp, poolid, epoch, state->mtime, params.remove_objs, y);
			 ...
			 const bool force)
{
  ...
  return del_op.delete_obj(null_yield, dpp, force);
}
                            ...
                            optional_yield y, bool assume_noent)
{
  ...
  do {
    ret = get_obj_state_impl(dpp, rctx, bucket_info, obj, psm,
			    ...
			    const bool force)
{
  ...
  *plast_ver = last_ver;
  ...
  for (list<cls_rgw_obj_key>::iterator liter = remove_instances.begin();
    ...
    int ret = delete_obj(dpp, obj_ctx, bucket_info, obj_instance, 0, null_verid, RGW_BILOG_FLAG_VERSIONED_OP, ceph::real_time(), zones_trace, force);
    if (ret < 0 && ret != -ENOENT) {
 ...
 */
	int RGWRados::update_olh(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWObjState *state, RGWBucketInfo& bucket_info, const rgw_obj& obj, rgw_zone_set *zones_trace, bool null_verid, const bool force)
{
  ...
  do {
    ...
    ret = apply_olh_log(dpp, obj_ctx, *state, bucket_info, obj, state->olh_tag, log, &ver_marker, null_verid, zones_trace, force);
    if (ret < 0) {
      return ret;
    }
  } while (is_truncated);
...
int RGWRados::unlink_obj_instance(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& target_obj,
                                  uint64_t olh_epoch, optional_yield y, uint16_t bilog_flags, bool null_verid, rgw_zone_set *zones_trace, const bool force)
{
  string op_tag;
  ...
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    if (ret < 0) {
      ...
      // object from being cleaned by another thread that was deleting the last
      // existing version. We invoke a best-effort update_olh here to handle this case.
      int r = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, zones_trace, null_verid, force);
      if (r < 0 && r != -ECANCELED) {
        ldpp_dout(dpp, 20) << "update_olh() target_obj=" << olh_obj << " returned " << r << dendl;
      }
      return ret;
  ...
  ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, zones_trace, null_verid, force);
  if (ret == -ECANCELED) { /* already did what we needed, no need to retry, raced with another user */
    return 0;
  }
  if (ret < 0) {
