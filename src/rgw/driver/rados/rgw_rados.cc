  ...
  for (uint16_t attempt = 1; /* empty */; ++attempt) {
	...
	": considering entry " << entry.key << dendl;
	  ...
	  // We're in the namespace range and we're enforcing an empty
 	  // name space, therefore we can skip past the rest of the
 	  // namespaced entries. Namespaces are demarked by
 	  // underscores before and after the namespace name. Regular
 	  ...
 	  // before underscore (e.g., uppercase letters) and those
	  ...
	  if (obj.ns[0] < '_') {
	    // we'll see if we can skip to the region of
	    // non-namespaced entries that begin with an underscore,
	    // as the underscore is escaped (i.e., "__foobar")
	    potential_marker = rgw_obj_index_key("_^\xFF");
	  } else {
	    // we're passed the double underscore region, so we can
	    // skip past the underscore region
	    potential_marker = rgw_obj_index_key("_\xFF");
	  }
