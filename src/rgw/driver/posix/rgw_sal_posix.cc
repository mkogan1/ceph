		       ...
		       optional_yield y)
{
  ...
  if (direct_io) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "URING: File::write posix_direct_write failed: "
					...
					int64_t end, RGWGetDataCB* cb, optional_yield y)
{
  ...
  if (posix_try_use_uring(dpp, y, /*nsfs=*/false)) {
    ...
    if (fd >= 0) {
      ...
      if (r < 0) {
        ldpp_dout(dpp, 1) << "URING: POSIXReadOp::iterate failed: "
  ...
  auto result = mc->list_parts(
	...
	[&](const char* name) -> int {
	  ...
	  if (pf->read_attrs(dpp, y, attrs) == 0) {
	    POSIXUploadPartInfo upi;
	    if (decode_attr(attrs, RGW_POSIX_ATTR_MPUPLOAD, upi) == 0) {
...
int POSIXMultipartWriter::process(bufferlist&& data, uint64_t offset)
{
  if (posix_try_use_uring(dpp, yield, /*nsfs=*/false) && part_file &&
    ...
    if (r < 0) {
      ldpp_dout(dpp, 1) << "URING: POSIXMultipartWriter::process failed: "
                        << cpp_strerror(-r) << " (" << r << ")" << dendl;
    }
    return r;
  }
                       ...
                       uint32_t flags)
{
  ...
  if (uring_write) {
    ...
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "URING: POSIXMultipartWriter::complete drain failed: "
                        << cpp_strerror(-ret) << " (" << ret << ")" << dendl;
      return ret;
    }
  } else {
...
int POSIXAtomicWriter::process(bufferlist&& data, uint64_t offset)
{
  ...
  if (posix_try_use_uring(dpp, yield, /*nsfs=*/false) && fd >= 0) {
    ...
    int r = uring_write->process(std::move(data), offset);
    if (r < 0) {
      ldpp_dout(dpp, 1) << "URING: POSIXAtomicWriter::process failed: "
                        << cpp_strerror(-r) << " (" << r << ")" << dendl;
    }
    return r;
  }
                       ...
                       uint32_t flags)
{
  ...
  if (uring_write) {
    ret = uring_write->drain();
    if (ret < 0) {
      ldpp_dout(dpp, 1) << "URING: POSIXAtomicWriter::complete drain failed: "
                        << cpp_strerror(-ret) << " (" << ret << ")" << dendl;
      return ret;
    }
  } else {
