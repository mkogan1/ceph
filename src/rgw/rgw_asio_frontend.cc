 ...
 public:
  StreamIO(CephContext *cct, Stream& stream, rgw::asio::parser_type& parser,
      ...
      : ClientIO(parser, is_ssl, local_endpoint, remote_endpoint),
        cct(cct), stream(stream), yield(yield), buffer(buffer), must_close(must_close), request_timeout(request_timeout)
  {}
...
void AsioFrontend::accept(Listener& l, boost::system::error_code ec)
{
  ...
  if (l.use_ssl) {
    spawn::spawn(context,
      [this, s=std::move(stream), ic=l.ignore_bad_content_length] (spawn::yield_context yield) mutable {
