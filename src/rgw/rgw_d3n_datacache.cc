...
void D3nDataCache::d3n_libaio_write_completion_cb(D3nCacheAioWriteRequest* c)
{
  ...
  c->c_blk->size_in_bytes = c->cb->aio_nbytes;
  ldout(cct, 5) << "Mandy added size_in_bytes to c_blk is: " << c->c_blk->size_in_bytes << dendl;
  ...
  ldout(cct, 5) << "Mandy pushed back hosts_list to c_blk, first is " << c->c_blk->hosts_list[0] << dendl;
  c->c_blk->c_obj.obj_name = c->oid;
  ldout(cct, 5) << "Mandy added c_obj.obj_name to c_blk is: " << c->c_blk->c_obj.obj_name << dendl;
...
class CacheThreadPool { //PORT THIS
  public:
    ...
    void addTask(Task *nt) {
      //ldout(cct, 1) << "D4N: Task added " <<  dendl;
...
void D3nDataCache::submit_remote_req(RemoteRequest *c){
  ...
  ldout(cct, 1) << "submit_remote_req, adding task " <<  dendl;

  tp->addTask(new RemoteS3Request(c, cct));
  ldout(cct, 1) << "submit_remote_req, task added " << dendl;
