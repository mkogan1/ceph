...
class RateLimiterEntry {
  ...
  */
                       ...
                       const RGWRateLimitInfo* info)
  {
    ...
    {
      const size_t accumulation_interval = g_ceph_context->_conf->rgw_ratelimit_accumulation_interval_seconds;
...
class RateLimiter {
  ...
  static OpType op_type(const std::string_view method, const std::string_view resource) {
    const bool ratelimit_list_and_delete = g_ceph_context->_conf->rgw_enable_ratelimit_list_and_delete;

    auto contains_any = [](std::string_view s, auto&&... patterns) {
      return ((s.find(patterns) != std::string::npos) || ...);
    };
    if (ratelimit_list_and_delete && method == "GET" &&
    ...
    if (ratelimit_list_and_delete && method == "DELETE") {
