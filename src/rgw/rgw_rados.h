...
struct objexp_hint_entry {
  ...
  void dump(Formatter *f) const;
  void generate_test_instances(list<objexp_hint_entry*>& o);
};
...
public:
  ...
  public:
    ...
    public:
      ...
      int complete_del(int64_t poolid, uint64_t epoch,
                       ...
                       list<rgw_obj_index_key> *remove_objs);
      int cancel(std::list<rgw_obj_index_key> *remove_objs);
  ...
  int olh_init_modification(RGWBucketInfo& bucket_info, RGWObjState& state, const rgw_obj& olh_obj, string *op_tag);
  ...
  int remove_olh_pending_entries(RGWBucketInfo& bucket_info, RGWObjState& state, const rgw_obj& olh_obj, map<string, bufferlist>& pending_attrs);
  int follow_olh(RGWBucketInfo& bucket_info, RGWObjectCtx& ctx, RGWObjState *state, const rgw_obj& olh_obj, rgw_obj *target);
  int get_olh(RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWOLHInfo *olh);
...
public:
  ...
  int fix_head_obj_locator(RGWBucketInfo& bucket_info, bool copy_obj, bool remove_bad, rgw_obj_key& key);
...
public:
  ...
  static RGWRados *get_storage(CephContext *cct, bool use_gc_thread, bool use_lc_thread, bool quota_threads,
    ...
    RGWRados *store = init_storage_provider(cct, use_gc_thread, use_lc_thread, quota_threads, run_sync_thread,
					    run_reshard_thread, use_cache, use_gc);
    return store;
  }
  ...
  static RGWRados *init_storage_provider(CephContext *cct, bool use_gc_thread, bool use_lc_thread, bool quota_threads, bool run_sync_thread, bool run_reshard_thread, bool use_metadata_cache, bool use_gc);
