...
public:
  ...
  int reindex_obj(RGWBucketInfo& bucket_info,
		  const rgw_obj& obj,
		  const DoutPrefixProvider* dpp,
		  optional_yield y);
  ...
  int set_olh(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& target_obj, bool delete_marker, rgw_bucket_dir_entry_meta *meta,
              ...
              optional_yield y, rgw_zone_set *zones_trace = nullptr, bool log_data_change = false, bool skip_olh_obj_update = false);
...
public:
  ...
  int remove_objs_from_index(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, const std::list<rgw_obj_index_key>& oid_list);
