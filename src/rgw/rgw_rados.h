...
public:
  ...
  struct BucketShard {
    ...
    int init(const rgw_bucket& _bucket, int sid, std::optional<rgw::bucket_index_layout_generation> current_layout, RGWBucketInfo* out, const DoutPrefixProvider *dpp);
   ...
   * Returns: 0 on success, -ERR# otherwise.
   */
  int set_attr(const DoutPrefixProvider *dpp, 
	       RGWObjectCtx *ctx,
  ...
  int get_obj_state(const DoutPrefixProvider *dpp,
		    RGWObjectCtx *rctx,
                    RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWObjState **state,
                    bool follow_olh, optional_yield y, bool assume_noent = false);
  int get_obj_state(const DoutPrefixProvider *dpp,
  ...
  int bucket_index_link_olh(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, RGWObjState& olh_state,
  ...
  int bucket_index_unlink_instance(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, const rgw_obj& obj_instance, const std::string& op_tag, const std::string& olh_tag, uint64_t olh_epoch, rgw_zone_set *zones_trace = nullptr);
  int bucket_index_read_olh_log(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, RGWObjState& state, const rgw_obj& obj_instance, uint64_t ver_marker,
  ...
  void check_pending_olh_entries(std::map<std::string, ceph::bufferlist>& pending_entries,
  ...
  int follow_olh(const DoutPrefixProvider *dpp,
		 RGWBucketInfo& bucket_info,
                 RGWObjectCtx& ctx, RGWObjState *state,
                 const rgw_obj& olh_obj, rgw_obj *target);
  int get_olh(const DoutPrefixProvider *dpp,
	      RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWOLHInfo *olh);
...
public:
  ...
  int cls_bucket_head(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, const rgw::bucket_index_layout_generation& idx_layout, int shard_id, std::vector<rgw_bucket_dir_header>& headers, std::map<int, std::string> *bucket_instance_ids = NULL);
  ...
  int defer_gc(const DoutPrefixProvider *dpp, 
	       RGWObjectCtx *rctx,
  ...
  int fix_head_obj_locator(const DoutPrefixProvider *dpp,
			   const RGWBucketInfo& bucket_info, bool copy_obj, bool remove_bad, rgw_obj_key& key);
  int fix_tail_obj_locator(const DoutPrefixProvider *dpp,
			   RGWBucketInfo& bucket_info,
