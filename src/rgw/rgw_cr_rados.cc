...
RGWRadosGetOmapValsCR::RGWRadosGetOmapValsCR(RGWRados *_store,
                      const rgw_raw_obj& _obj,
                      const string& _marker,
                      int _max_entries,
                      ResultPtr _result)
...
RGWRadosRemoveOidCR::RGWRadosRemoveOidCR(RGWRados* store,
...
int RGWRadosRemoveOidCR::send_request()
{
  librados::ObjectWriteOperation op;
  if (objv_tracker) {
    objv_tracker->prepare_op_for_write(&op);
...
int RGWAsyncGetBucketInstanceInfo::_send_request()
{
  ...
  int r = store->get_bucket_instance_from_oid(obj_ctx, oid, bucket_info, nullptr,
					      &attrs);
  if (r < 0) {
...
{
  bs.init(bucket_info, generation, shard_id);
}
...
int RGWDataPostNotifyCR::operate()
{
  reenter(this) {
