...
class FormPostSignatureT: public rgw::auth::swift::FormatSignature<H,O>
{
  using UCHARPTR = const unsigned char*;
  using base_t = rgw::auth::swift::SignatureHelperT<H>;
...
RGWFormPost::SignatureHelper* RGWFormPost::SignatureHelper::get_sig_helper(std::string_view x) {
  size_t pos = x.find(':');
  if (pos == x.npos || pos <= 0) {
    switch(x.length()) {
    case CEPH_CRYPTO_HMACSHA1_DIGESTSIZE*2:
      return new SignatureHelper_x<ceph::crypto::HMACSHA1,false>();
    case CEPH_CRYPTO_HMACSHA256_DIGESTSIZE*2:
      return new SignatureHelper_x<ceph::crypto::HMACSHA256,false>();
    case CEPH_CRYPTO_HMACSHA512_DIGESTSIZE*2:
      return new SignatureHelper_x<ceph::crypto::HMACSHA512,false>();
    }
    return new BadSignatureHelper();
  }
  std::string_view type { x.substr(0,pos) };
  if (type == "sha1") {
    return new SignatureHelper_x<ceph::crypto::HMACSHA1,true>();
  } else if (type == "sha256") {
    return new SignatureHelper_x<ceph::crypto::HMACSHA256,true>();
  } else if (type == "sha512") {
    return new SignatureHelper_x<ceph::crypto::HMACSHA512,true>();
  }
  return new BadSignatureHelper();
};
...
bool RGWFormPost::is_integral()
{
  ...
  for (const auto& kv : s->user->get_info().temp_url_keys) {
    ...
    SignatureHelper* sig_helper{ RGWFormPost::SignatureHelper::get_sig_helper(form_signature) };
