...
int RGWRole::store_info(bool exclusive)
{
  ...
  if (!this->tags.empty()) {
    ...
    return rgw_put_system_obj(store, store->svc.zone->get_zone_params().roles_pool, oid,
                bl, exclusive, NULL, real_time(), &attrs);
  }
  return rgw_put_system_obj(store, store->svc.zone->get_zone_params().roles_pool, oid,
...
int RGWRole::read_info()
{
  ...
  map<string, bufferlist> attrs;  
  int ret = rgw_get_system_obj(store, obj_ctx, pool, oid, bl, NULL, NULL, &attrs, NULL, boost::none, true);
  if (ret < 0) {
  ...
  if (it != attrs.end()) {
    ...
    try {
      ...
      ldout(cct, 0) << "ERROR: failed to decode attrs" << id << dendl;
      return -EIO;
    }
  }
