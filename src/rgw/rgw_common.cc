...
int verify_bucket_owner_or_policy(struct req_state* const s,
				  const uint64_t op)
{
  auto identity_policy_res = eval_identity_or_session_policies(s->iam_user_policies, s->env, boost::none, op, ARN(s->bucket));
  if (identity_policy_res == Effect::Deny) {
    return -EACCES;
  }
  ...
  auto e = eval_or_pass(s->iam_policy,
			...
			op, ARN(s->bucket), princ_type);
  if (e == Effect::Deny) {
    return -EACCES;
  }

  if (!s->session_policies.empty()) {
    auto session_policy_res = eval_identity_or_session_policies(s->session_policies, s->env, boost::none, op, ARN(s->bucket));
    if (session_policy_res == Effect::Deny) {
        return -EACCES;
    }
    if (princ_type == rgw::IAM::PolicyPrincipal::Role) {
                              ...
                              const uint64_t op)
{
  ...
  rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
  auto r = eval_or_pass(bucket_policy, s->env, *s->auth.identity, op, ARN(obj), princ_type);
  if (r == Effect::Deny)
    return false;
