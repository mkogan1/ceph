...
class BucketCleanIndexCollectCR : public RGWShardCollectCR {
 ...
 public:
  BucketCleanIndexCollectCR(RGWRados* store,
			    const RGWBucketInfo& bucket_info,
			    rgw::bucket_index_layout_generation index)
    : RGWShardCollectCR(store->ctx(), MAX_CONCURRENT_SHARDS),
      store(store), bucket_info(bucket_info),
  ...
  bool spawn_next() override {
    if (shard < num_shards) {
      ...
      bs.init(bucket_info, index, shard);
      spawn(new RGWRadosRemoveOidCR(store, std::move(bs.index_ctx),
				    std::move(bs.bucket_obj)),
	    false);
      ++shard;
      return true;
    } else {
...
int BucketTrimInstanceCR::operate()
{
  reenter(this) {
    ...
    yield call(new RGWBucketGetSyncPolicyHandlerCR(store->get_async_rados(),
                                                   store,
                                                   get_policy_params,
                                                   source_policy));
    if (retcode < 0) {
    ...
    if (retcode < 0) {
      ldout(cct, 4) << "failed to find minimum generation" << dendl;
      return set_cr_error(retcode);
    }
    retcode = maybe_remove_generation();
    if (retcode < 0) {
      ldout(cct, 4) << "error removing old generation from log: "
	...
	if (retcode == -ECANCELED) {
	  yield call(new RGWGetBucketInstanceInfoCR(
		       ...
		       &(clean_info->first), nullptr));
	  if (retcode < 0) {
	    ldout(cct, 0) << "failed to get bucket info: "
			  << cpp_strerror(retcode) << dendl;
	    return set_cr_error(retcode);
	  }
	...
	if (retcode < 0) {
	  ldout(cct, 0) << "failed to put bucket info: "
			<< cpp_strerror(retcode) << dendl;
	  return set_cr_error(retcode);
	}
	       ...
	       std::string_view start_marker, std::string_view end_marker)
{
  ...
  if (r < 0) {
    ldout(store->ctx(), 5) << __PRETTY_FUNCTION__ << ":" << __LINE__
			   << "ERROR: trim_bi_log_entries returned r=" << r << dendl;
  }
  return r;
}
