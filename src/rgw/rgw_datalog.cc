				 ...
				 int shard_id)
{
  ...
  if (now < status->cur_expiration) {
    ldout(cct, 20) << "RGWDataChangesLog::add_entry() entry added within the data log window. bucket.name="
      << bucket.name << " shard_id=" << shard_id << dendl;
  ...
  do {
    ...
    ldout(cct, 20) << "RGWDataChangesLog::add_entry() sending update with bucket.name=" << bucket.name
		   << " shard_id=" << shard_id << " gen=" << gen.gen << " now=" << now
...
int DataLogBackends::trim_generations(std::optional<uint64_t>& through) {
  if (size() != 1) {
    ...
    auto r = empty_to(*highest, null_yield);
    if (r < 0) {
      return r;
    }
  }

  return remove_empty(null_yield);
}
