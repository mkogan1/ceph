  ...
  int operate() override {
	...
	snprintf(buf, sizeof(buf), "%d", shard_id);
        ...
        if (sync_env->counters) {
          timer_started = ceph::real_clock::now();
        }
        int ret = http_op->aio_read();
        if (ret < 0) {
          ldout(sync_env->cct, 0) << "ERROR: failed to read from " << p << dendl;
      ...
      yield {
        if (sync_env->counters) {
          auto latency = ceph::real_clock::now() - timer_started;
          sync_env->counters->tinc(sync_counters::l_poll, latency);
        }
...
public:
  ...
  bool can_do_op(const rgw_obj_key& key, bool is_olh) {
    ...
    if (is_olh && pending_olh.count(key.name)) {
      ldout(sync_env->cct, 20) << "sync of " << key << " waiting for pending olh op" << dendl;
      return false;
    }
...
int DataLogTrimCR::operate()
{
  reenter(this) {
    ...
    yield {
      ...
      for (auto& c : store->zone_data_notify_to_map) {
