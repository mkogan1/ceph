...
#define OMAP_GET_MAX_ENTRIES 100
    ...
    reenter(&full_cr) {
      ...
      yield {
        ...
        auto store = sync_env->store;
        call(new RGWSimpleRadosWriteCR<rgw_data_sync_marker>(sync_env->async_rados, sync_env->store->svc.sysobj,
                                                             rgw_raw_obj(pool, status_oid),
                                                             sync_marker));
      }
      if (retcode < 0) {
    ...
    reenter(this) {
      ...
      yield {
        ...
        if (info.syncstopped) {
          ...
          call(new RGWSimpleRadosWriteAttrsCR(sync_env->async_rados, sync_env->store->svc.sysobj, obj, attrs, &objv_tracker));
	}
      }
      if (info.syncstopped) {
        retcode = -ENOENT;
...
public:
  ...
  RGWCoroutine *store_marker(const rgw_obj_key& new_marker, uint64_t index_pos, const real_time& timestamp) override {
    ...
    return new RGWSimpleRadosWriteAttrsCR(sync_env->async_rados, store->svc.sysobj,
                                          rgw_raw_obj(sync_env->store->svc.zone->get_zone_params().log_pool, marker_oid),
                                          attrs, &objv_tracker);
  }
...
public:
  RGWBucketSyncSingleEntryCR(RGWDataSyncCtx *_sc,
    ...
    zones_trace = _zones_trace;
    zones_trace.insert(sync_env->store->svc.zone->get_zone().id, _sync_pipe.info.dest_bs.get_key());
  }
...
public:
  RGWBucketShardIncrementalSyncCR(RGWDataSyncCtx *_sc,
    ...
    : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
      sync_pipe(_sync_pipe), bs(_sync_pipe.info.source_bs),
      lease_cr(std::move(lease_cr)), sync_info(sync_info),
      zone_id(sync_env->store->svc.zone->get_zone().id),
