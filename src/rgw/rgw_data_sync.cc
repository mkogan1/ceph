...
public:
  RGWRunBucketSyncCoroutine(RGWDataSyncEnv *_sync_env,
			    boost::intrusive_ptr<const RGWContinuousLeaseCR> lease_cr,
			    const rgw_bucket_shard& bs, const RGWSyncTraceNodeRef& _tn_parent,
			    ceph::real_time* progress)
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env),
      lease_cr(std::move(lease_cr)), bs(bs), progress(progress),
...
public:
  RGWDataSyncSingleEntryCR(RGWDataSyncEnv* _sync_env, rgw::bucket_sync::Handle state,
    ...
    set_description() << "data sync single entry (source_zone=" << _sync_env->source_zone << ") " << obligation;
  ...
  int operate() override {
	...
	obligation_counter = state->counter;
		...
		state->progress_timestamp < state->obligation->timestamp) &&
          ...
          yield call(new RGWRunBucketSyncCoroutine(sync_env, lease_cr,
	...
	tn->log(10, SSTR("sync finished on " << bucket_shard_str{state->key}
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          yield call(sync_env->error_logger->log_error_cr(sync_env->conn->get_remote_id(), "data", complete->key,
...
public:
  RGWDataSyncShardCR(RGWDataSyncEnv *_sync_env,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), pool(_pool),
      ...
      truncated(false), total_entries(0), reset_backoff(NULL),
      lease_cr(nullptr), lease_stack(nullptr),
      status_oid(RGWDataSyncStatusManager::shard_obj_name(sync_env->source_zone,
							  shard_id)),
      error_repo(pool, status_oid + ".retry"),
...
public:
  ...
  void wakeup(int shard_id, set<string>& keys) {
    std::lock_guard<ceph::mutex> l(shard_crs_lock);
    ...
    reenter(this) {
      ...
      yield {
        ...
        if (info.syncstopped) {
          ...
          call(new RGWSimpleRadosWriteAttrsCR(sync_env->async_rados, store->svc.sysobj, obj, attrs, &objv_tracker));
        }
      }
      if (info.syncstopped) {
        retcode = -ENOENT;
...
RGWCoroutine *RGWRemoteBucketLog::init_sync_status_cr(RGWObjVersionTracker& objv_tracker)
{
  return new RGWInitBucketShardSyncStatusCoroutine(&sync_env, bs, init_status, objv_tracker);
}
...
{
  return new RGWReadBucketSyncStatusCoroutine(&sync_env, bs, sync_status, nullptr);
}
...
public:
  RGWBucketFullSyncShardMarkerTrack(RGWDataSyncEnv *_sync_env,
    ...
    : RGWSyncShardMarkerTrack(BUCKET_SYNC_UPDATE_MARKER_WINDOW),
      sync_env(_sync_env), marker_oid(_marker_oid),
      sync_marker(_marker), objv_tracker(objv_tracker) {}
 ...
 public:
  ...
  int operate() {
    reenter(this) {
      ...
      yield call(new RGWSimpleRadosWriteAttrsCR(sync_env->async_rados,
...
public:
  RGWBucketIncSyncShardMarkerTrack(RGWDataSyncEnv *_sync_env,
    ...
    : RGWSyncShardMarkerTrack(BUCKET_SYNC_UPDATE_MARKER_WINDOW),
      ...
      sync_marker(_marker), objv_tracker(objv_tracker), stable_timestamp(stable_timestamp) {}
...
public:
  RGWBucketShardFullSyncCR(RGWDataSyncEnv *_sync_env, const rgw_bucket_shard& bs,
			   ...
			   RGWObjVersionTracker& objv_tracker)
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      bucket_info(_bucket_info), lease_cr(std::move(lease_cr)), sync_info(sync_info),
      status_oid(status_oid),
      tn(sync_env->sync_tracer->add_node(tn_parent, "full_sync",
                                         SSTR(bucket_shard_str{bs}))),
      marker_tracker(sync_env, status_oid, sync_info.full_marker, objv_tracker) {
...
public:
  RGWBucketShardIncrementalSyncCR(RGWDataSyncEnv *_sync_env,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      bucket_info(_bucket_info), lease_cr(std::move(lease_cr)), sync_info(sync_info),
      zone_id(_sync_env->store->svc.zone->get_zone().id),
...
{
  reenter(this) {
    yield call(new RGWReadBucketSyncStatusCoroutine(sync_env, bs, &sync_status, &objv_tracker));
    if (retcode < 0 && retcode != -ENOENT) {
    ...
    do {
      if (sync_status.state == rgw_bucket_shard_sync_info::StateInit) {
        yield call(new RGWInitBucketShardSyncStatusCoroutine(sync_env, bs, sync_status, objv_tracker));
        if (retcode == -ENOENT) {
...
{
  return new RGWRunBucketSyncCoroutine(&sync_env, nullptr, bs, sync_env.sync_tracer->root_node, nullptr);
}
...
int RGWBucketSyncStatusManager::init_sync_status()
{
  ...
  for (auto iter = source_logs.begin(); iter != source_logs.end(); ++iter) {
    RGWCoroutinesStack *stack = new RGWCoroutinesStack(store->ctx(), &cr_mgr);
    RGWRemoteBucketLog *l = iter->second;
    objvs.emplace_back();
    stack->call(l->init_sync_status_cr(objvs.back()));
...
class RGWCollectBucketSyncStatusCR : public RGWShardCollectCR {
 ...
 public:
  ...
  bool spawn_next() override {
    ...
    spawn(new RGWReadBucketSyncStatusCoroutine(env, bs, &*i, nullptr), false);
    ++i;
    ++bs.shard_id;
    return true;
  }
