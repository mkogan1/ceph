  ...
  RGWDataSyncSingleEntryCR(RGWDataSyncEnv* _sync_env,
			   const string& _raw_key, const string& _entry_marker,
                           ...
                           RGWOmapAppend *_error_repo, bool _remove_from_repo, const RGWSyncTraceNodeRef& _tn_parent) : RGWCoroutine(_sync_env->cct),
                                                      sync_env(_sync_env), raw_key(_raw_key), entry_marker(_entry_marker),
      ...
      do {
        ...
        for (; iter != entries.end(); ++iter) {
          ...
          if (!marker_tracker->start(iter->first, total_entries, entry_timestamp)) {
            ...
            yield spawn(new RGWDataSyncSingleEntryCR(sync_env, iter->first, iter->first, entry_timestamp, marker_tracker, error_repo, false, tn), false);
          }
      ...
      do {
        ...
        for (modified_iter = current_modified.begin(); modified_iter != current_modified.end(); ++modified_iter) {
          yield {
            ...
            spawn(new RGWDataSyncSingleEntryCR(sync_env, *modified_iter, string(), ceph::real_time{}, marker_tracker, nullptr, false, tn), false);
          }
        }
        ...
        if (error_retry_time <= ceph::coarse_real_clock::now()) {
          ...
          for (; iter != error_entries.end(); ++iter) {
            ...
            spawn(new RGWDataSyncSingleEntryCR(sync_env, error_marker, error_marker,
        ...
        for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
          ...
          if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
            ...
            spawn(new RGWDataSyncSingleEntryCR(sync_env, log_iter->entry.key, log_iter->log_id,
