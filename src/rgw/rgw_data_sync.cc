...
public:
  RGWRunBucketSourcesSyncCR(RGWDataSyncEnv *_sync_env,
    ...
    if (target_bs) {
      ...
      sources_obj = RGWBucketSyncPeersManager::sync_sources_obj(_sync_env->store->svc.zone, *target_bucket);
    }
    if (source_bs) {
  ...
  reenter(this) {
    ...
    for (siter = pipes.begin(); siter != pipes.end(); ++siter) {
...

      {
        ...
        if (last_zone != szone) {
          conn = sync_env->store->svc.zone->get_zone_conn_by_id(szone);
          if (!conn) {
  ...
  reenter(this) {
    ...
    if (target_bucket) {
      yield call(new RGWSimpleRadosReadCR<rgw_bucket_sync_sources_local_info>(sync_env->async_rados,
                                                                              ...
                                                                              RGWBucketSyncPeersManager::sync_sources_obj(sync_env->store->svc.zone, *target_bucket),
                                                                              &sources_local_info));
      if (retcode < 0 &&
          retcode != -ENOENT) {
        return set_cr_error(retcode);
    ...
    if (source_bucket && source_zone) {
      yield call(new RGWSimpleRadosReadCR<rgw_bucket_sync_sources_local_info>(sync_env->async_rados,
                                                                              sync_env->store->svc.sysobj,
                                                                              RGWBucketSyncPeersManager::sync_targets_obj(sync_env->store->svc.zone, *source_zone, *source_bucket),
                                                                              &sources_local_info));
      if (retcode < 0 &&
          retcode != -ENOENT) {
        return set_cr_error(retcode);
