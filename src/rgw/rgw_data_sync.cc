...
RGWCoroutine *RGWDefaultDataSyncModule::sync_object(RGWDataSyncCtx *sc, rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key, std::optional<uint64_t> versioned_epoch, rgw_zone_set *zones_trace)
{
  ...
  return new RGWObjFetchCR(sc, sync_pipe, key, std::nullopt, versioned_epoch, stat_follow_olh, zones_trace);
}
...
RGWCoroutine *RGWArchiveDataSyncModule::sync_object(RGWDataSyncCtx *sc, rgw_bucket_sync_pipe& sync_pipe, rgw_obj_key& key, std::optional<uint64_t> versioned_epoch, rgw_zone_set *zones_trace)
{
  ...
  return new RGWObjFetchCR(sc, sync_pipe, key, dest_key, versioned_epoch, stat_follow_olh, zones_trace);
}
	...
	yield call(new InitBucketFullSyncStatusCR(sc, sync_pair, status_obj,
	  ...
	  source_bs = sync_pair.source_bs;
	    ...
	    pool = sc->env->svc->zone->get_zone_params().log_pool;
	      ...
	      // use the error repo and sync status timestamp from the datalog shard corresponding to source_bs
              error_repo = datalog_oid_for_error_repo(sc, sc->env->store,
			   pool, source_bs);
              yield call(rgw::error_repo::write_cr(sc->env->store->svc()->rados, error_repo,
