	...
	if (retcode < 0) {
	  ldout(cct, 0)
        ...
        for (iter = result.keys.begin(); iter != result.keys.end(); ++iter) {
          ldout(cct, 20) << "list metadata: section=bucket.instance key="
			 << *iter << dendl;
	  key = *iter;
	  ...
	  if (retcode < 0) {
	    ldout(cct, 0) << "ERROR: failed to fetch metadata for key: "
			  << key << dendl;
	    return set_cr_error(retcode);
	  }
	  ...
	  yield entries_index->append(
	    fmt::format("{}:{}", key, 0),
	    sync_env->store->data_log->get_log_shard_id(
      ...
      if (!failed) {
        for (auto iter = sync_status->sync_markers.begin();
	     ...
	     ++iter) {
		  ...
		  rgw_raw_obj(sync_env->store->svc.zone->get_zone_params().log_pool,
        ...
        yield call(sync_env->error_logger->log_error_cr(
		     sc->conn->get_remote_id(), "data.init", "",
  ...
  reenter(this) {
    ...
    if (retcode == -ENOENT) {
      *status = rgw_bucket_shard_sync_info();
      ldout(cct, 20) << "INFO: new bucket shard info for oid=" << oid << dendl;
      return set_cr_done();
    }
    if (retcode < 0) {
    ...
    ldout(cct, 20) << "INFO: existing bucket shard info for oid=" << oid << dendl;
    return set_cr_done();
  }
  return 0;
}
...
public:
  RemoveBucketShardStatusCR(RGWDataSyncCtx* sc,
    ...
    : RGWCoroutine(sc->cct), sc(sc), sync_env(sc->env),
      sync_pair(sync_pair),
      obj(sync_env->store->svc.zone->get_zone_params().log_pool, 
  ...
  int operate() override {
	...
	if (check_compat) {
	  ...
	  if (source_info.layout.logs.empty() ||
	    ...
	    ldout(cct, 20) << "no generation zero when checking compatibility" << dendl;
	    no_zero = true;
	  } else if (auto& log = source_info.layout.logs.front();
                     log.layout.type != rgw::BucketLogType::InIndex) {
	    ldout(cct, 20) << "unrecognized log layout type when checking compatibility " << log.layout.type << dendl;
	    no_zero = true;
	  }
	  if (!no_zero) {
	    yield {
  ...
  int operate() override
  {
    reenter(this) {
      do {
        ...
        if (retcode < 0) {
          ldout(cct, 20) << "RGWBucketShardIsDoneCR failed to read bucket shard status object: " <<
        ...
        if (bucket_status.state != BucketSyncState::Incremental) {
          ...
          ldout(cct, 20) << "RGWBucketShardIsDoneCR found sync state = " << bucket_status.state <<
        ...
        yield {
          ...
          if (std::all_of(done.begin(), done.end(),
            ...
            ldout(cct, 20) << "RGWBucketShardIsDoneCR generation bumped from: "  <<
        ...
        if (retcode < 0 && retcode != -ECANCELED) {
          ldout(cct, 20) << "RGWBucketShardIsDoneCR failed to write bucket sync status object: " <<
            bucket_status_obj.oid << ". error: " << cpp_strerror(retcode) << dendl;
          return set_cr_error(retcode);
        } else if (retcode >= 0) {
          ldout(cct, 20) << "RGWBucketShardIsDoneCR done for shard_id=" << shard_id << " wrote bucket sync status object: " <<
...
int RGWBucketShardIncrementalSyncCR::operate()
{
  ...
  reenter(this) {
    do {
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          ldout(cct, 20) << "syncstop at: " << e.timestamp << ". marker: " << e.id << dendl;
          syncstopped = true;
          entries_end = std::next(entries_iter); // stop after this entry
          break;
        }
        if (e.op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          ldout(cct, 20) << "syncstart at: " << e.timestamp << ". marker: " << e.id << dendl;
          continue;
        }
	...
	retcode = sync_env->store->get_raw_obj_ref(marker_tracker.get_obj(), &status_obj);
	if (retcode < 0) {
          return set_cr_error(retcode);
        }
        call(new RGWRadosRemoveOidCR(sync_env->store, std::move(status_obj.ioctx),
				     status_obj.obj.oid));
        if (retcode < 0) {
...
int RGWRunBucketSourcesSyncCR::operate()
{
  reenter(this) {
    ...
    ldpp_dout(sync_env->dpp, 20) << __func__ << "(): requested source_bs=" << sync_pair.source_bs << dendl;
...
int RGWBucketPipeSyncStatusManager::init()
{
  ...
  for (auto& pipe : pipes) {
    ...
    auto r = rgw_read_remote_bilog_info(conn, pipe.source.get_bucket_info().bucket,
					remote_info, remote_markers,
					null_yield);

    if (r < 0) {
      ldout(store->ctx(), 0) << __PRETTY_FUNCTION__ << ":" << __LINE__
