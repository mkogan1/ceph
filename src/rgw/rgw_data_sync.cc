...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
	...
	init_new_io(http_op);
        ...
        if (ret < 0) {
	  ldpp_dout(dpp, 0) << __PRETTY_FUNCTION__ << ": " << http_op->stamp()
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
	...
	snprintf(buf, sizeof(buf), "%d", shard_id);
        ...
        http_op = new RGWRESTReadResource(sc->conn, p, pairs, NULL, sync_env->http_manager);
	ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ": " << http_op->stamp()
			   << ": created" << dendl;
...
public:
  RGWDataIncrementalSyncFullObligationCR(RGWDataSyncCtx *_sc, rgw_bucket_shard& _source_bs,
					 const rgw_raw_obj& error_repo, std::string _error_marker,
                         ceph::real_time& _timestamp, RGWSyncTraceNodeRef& _tn)
    : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
      source_bs(_source_bs), error_repo(error_repo), error_marker(_error_marker), timestamp(_timestamp), tn(_tn) {
...
public:
  RGWDataSyncShardCR(RGWDataSyncCtx *_sc, rgw_pool& _pool,
                     ...
                     RGWSyncTraceNodeRef& _tn, RGWObjVersionTracker& objv,
		     bool *_reset_backoff)
    : RGWCoroutine(_sc->cct), sc(_sc), sync_env(_sc->env),
      pool(_pool), shard_id(_shard_id), sync_marker(_marker), sync_status(_sync_status),
      objv(objv), status_oid(RGWDataSyncStatusManager::shard_obj_name(sc->source_zone, shard_id)),
...
#define OMAP_GET_MAX_ENTRIES 100
  ...
  /* Reread data sync status to fech the latest marker and update objv */
        ...
        ldpp_dout(sync_env->dpp, 20) << __PRETTY_FUNCTION__ << " XXXXXXXXXXXXXXXXXXX before tmp_obv.read_version.ver=" << tmp_objv.read_version.ver << " , tmp_sync_marker.marker=" << tmp_sync_marker.marker << " , objv.read_version.ver=" << objv.read_version.ver <<" , sync_arker.marker=" << sync_marker.marker << dendl;
  ...
  yield call(new RGWSimpleRadosReadCR<rgw_data_sync_marker>(sync_env->dpp, sync_env->store,
    ...
    if (retcode < 0) {
      ...
      return set_cr_error(retcode);
        ...
        ldpp_dout(sync_env->dpp, 20) << __PRETTY_FUNCTION__ << " XXXXXXXXXXXXXXXXXXX after tmp_obv.read_version.ver=" << tmp_objv.read_version.ver << " , tmp_sync_marker.marker=" << tmp_sync_marker.marker << " , objv.read_version.ver=" << objv.read_version.ver <<" , sync_arker.marker=" << sync_marker.marker << dendl;
        ...
        if (error_retry_time <= ceph::coarse_real_clock::now()) {
          ...
          for (; iter != error_entries.end(); ++iter) {
            ...
            if (!gen) {
              ...
              spawn(new RGWDataIncrementalSyncFullObligationCR(sc, source_bs, error_repo, error_marker, entry_timestamp, tn), false);
            } else {
...
public:
  ...
  int operate(const DoutPrefixProvider *dpp) override {
	...
	yield call(new RGWInitDataSyncStatusCoroutine(sc, num_shards, instance_id, tn, &sync_status, init_lease, obj_version, objvs));
        if (retcode < 0) {
      ...
      if (init_lease &&
	  (rgw_data_sync_info::SyncState)sync_status.sync_info.state ==
	  rgw_data_sync_info::StateSync) {
	init_lease->go_down();
	drain_all();
	init_lease.reset();
...
public:
  ...
  RGWBucketInstanceMetadataHandlerBase *alloc_bucket_instance_meta_handler(rgw::sal::RGWRadosStore* store) override {
