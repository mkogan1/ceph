    ...
    reenter(this) {
      do {
        ...
        yield call(new RGWRunBucketSyncCoroutine(sync_env,
                                                 ...
                                                 tn, &progress));
	if (retcode == 0) {
	  tn->log(20, SSTR("RunBucketSync progress=" << progress));
	}
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          yield call(sync_env->error_logger->log_error_cr(sync_env->conn->get_remote_id(), "data", obligation.key,
...
public:
  RGWBucketShardIncrementalSyncCR(RGWDataSyncEnv *_sync_env,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      bucket_info(_bucket_info), lease_cr(lease_cr), sync_info(sync_info),
      marker_tracker(sync_env, status_oid, sync_info.inc_marker, stable_timestamp),
