                    ...
                    rgw_ep_info *pinfo)
{
  ...
  RGWObjVersionTracker* pobjv = &objv;
  ...
  if (update_entrypoint) {
    if (pinfo) {
      ...
      ret = store->get_bucket_entrypoint_info(obj_ctx,
		tenant_name, bucket_name, ep, pobjv, NULL, &attrs);
      if (ret < 0 && ret != -ENOENT) {
...
int rgw_remove_object(RGWRados *store, const RGWBucketInfo& bucket_info, const rgw_bucket& bucket, rgw_obj_key& key)
{
  ...
  RGWBucketInfo& ncbi = const_cast<RGWBucketInfo&>(bucket_info); 
  return store->delete_obj(rctx, ncbi, obj, ncbi.versioning_status());
}
...
int RGWBucket::chown(RGWBucketAdminOpState& op_state, const string& marker, std::string *err_msg)
{
  ...
  int ret = store->get_bucket_info(obj_ctx, tenant, bucket_name, bucket_info, NULL, &attrs);
  if (ret < 0) {
