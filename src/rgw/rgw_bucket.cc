                                ...
                                const DoutPrefixProvider *dpp)
{
  ...
  ret = store->getRados()->get_bucket_stats(dpp, info, info.layout.current_index, RGW_NO_SHARD, &bucket_ver, &master_ver, stats, NULL);
  if (ret < 0)
    return ret;
                                  ...
                                  std::string *err_msg)
{
  ...
  store->getRados()->cls_obj_set_bucket_tag_timeout(dpp, bucket_info, BUCKET_TAG_QUICK_TIMEOUT);
				  ...
				  bool warnings_only)
{
  ...
  for (const auto& user_id : user_ids) {
	...
	ret = store->getRados()->get_bucket_stats(dpp, info, index, RGW_NO_SHARD, &bucket_ver,
...
public:
  ...
  RGWBucketInstanceMetadataHandler(rgw::sal::RGWRadosStore* store)
    : store(store) {}
...
int RGWMetadataHandlerPut_BucketInstance::put_check(const DoutPrefixProvider *dpp)
{
  ...
  if (!exists || old_bci->info.bucket.bucket_id != bci.info.bucket.bucket_id) {
    ...
    if (bihandler->store->get_tier_type() == "archive") {
...
int RGWMetadataHandlerPut_BucketInstance::put_post(const DoutPrefixProvider *dpp)
{
  ...
  /* update lifecyle policy */
  {
    ...
    if (lc_it != bci.attrs.end()) {
      ...
      ret = lc->set_bucket_config(bucket->get_info(), bci.attrs, nullptr);
      if (ret < 0) {
      ...
      ret = lc->remove_bucket_config(bucket->get_info(), bci.attrs, false /* cannot merge attrs */);
      if (ret < 0) {
...
public:
  RGWArchiveBucketInstanceMetadataHandler(rgw::sal::RGWRadosStore* store)
...
RGWBucketInstanceMetadataHandlerBase* RGWBucketInstanceMetaHandlerAllocator::alloc(rgw::sal::RGWRadosStore* store)
{
...
RGWBucketInstanceMetadataHandlerBase* RGWArchiveBucketInstanceMetaHandlerAllocator::alloc(rgw::sal::RGWRadosStore* store)
{
