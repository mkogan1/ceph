				  ...
				  std::uint64_t tid)
{
  ...
  op.exec(fifo::op::CLASS, fifo::op::LIST_PART, in,
	  new list_entry_completion(cct, r_out, entries, more, full_part, tid));
  return op;
}
		       ...
		       std::uint64_t tid)
{
  ...
  std::string err;
  auto r = info->apply_update(update, &err);
  if (r < 0) {
    ldpp_dout(dpp, -1) << __PRETTY_FUNCTION__ << ":" << __LINE__
			    ...
			    std::uint64_t tid, optional_yield y)
{
  ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
		 << " entering: tid=" << tid << dendl;
  std::unique_lock l(m);
  std::vector jentries{info.next_journal_entry(new_part_num)};
  if (info.journal.find(new_part_num) != info.journal.end()) {
    l.unlock();
...
struct NewHeadPreparer : public Completion<NewHeadPreparer> {
  ...
  void handle_update(const DoutPrefixProvider *dpp, Ptr&& p, int r) {
    ...
    if (canceled) {
      ...
      auto head_part_num = f->info.head_part_num;
      auto version = f->info.version;
      auto found = iter != f->info.journal.cend() && iter->second.op == fifo::journal_entry::Op::set_head;
...
struct Trimmer : public Completion<Trimmer> {
  ...
  void handle(const DoutPrefixProvider *dpp, Ptr&& p, int r) {
	...
	ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ << ":" << __LINE__
		       << " pn=" << pn << " tid=" << tid << dendl;
	fifo->trim_part(dpp, pn++, max_part_size,
			false, tid, call(std::move(p)));
      } else {
	update = true;
	canceled = tail_part_num < part_num;
	fifo->trim_part(dpp, part_num, ofs, exclusive, tid,
			call(std::move(p)));
      }
      return;
    }
