...
public:
  RGWRadosRemoveOidCR(rgw::sal::RGWRadosStore* store,
...
public:
  RGWFetchRemoteObjCR(RGWAsyncRadosProcessor *_async_rados, rgw::sal::RGWRadosStore *_store,
    ...
    : RGWSimpleCoroutine(_store->ctx()), cct(_store->ctx()),
      ...
      stat_follow_olh(_stat_follow_olh), dpp(dpp) {}
  ...
  int send_request(const DoutPrefixProvider *dpp) override {
    ...
    source_zone, user_id, src_bucket, dest_placement_rule, dest_bucket_info,
                                     ...
                                     zones_trace, counters, stat_follow_olh, dpp);
    async_rados->queue(req);
    return 0;
  }
...
public:
  RGWContinuousLeaseCR(RGWAsyncRadosProcessor* async_rados,
    ...
    : RGWCoroutine(_store->ctx()), async_rados(async_rados), store(_store),
      ...
      interval(interval), caller(caller), latency(latency) {}
