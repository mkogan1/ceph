...
struct AWSSyncConfig {
  ...
  void expand_target(RGWDataSyncCtx *sc, const string& sid, const string& path, string *dest) {
      ...
      const RGWZoneGroup& zg = sc->env->store->svc.zone->get_zonegroup();
      apply_meta_param(path, "zonegroup", zg.get_name(), dest);
      apply_meta_param(path, "zonegroup_id", zg.get_id(), dest);

      const RGWZone& zone = sc->env->store->svc.zone->get_zone();
...
public:
  ...
  int operate() override {
    reenter(this) {
      ...
      source_conn = sync_env->store->svc.zone->get_zone_conn_by_id(sc->source_zone);
      if (!source_conn) {
