  ...
  if (qsr) {
    ...
    if (header_time) {
      ...
      if (!parse_rfc2616(req_date, &t) && !parse_iso8601(req_date, &t, &ns, false)) {
        dout(0) << "NOTICE: failed to parse date <" << req_date << "> for auth header" << dendl;
        return false;
      }
      if (t.tm_year < 70) {
                                        ...
                                        const size_t metabuf_len)
{
  ...
  char* data_field_end;
  static const boost::string_ref chunk_signature{"chunk-signature"};
  boost::string_ref signature;
