...
static inline ExtractResult mut_extract_helper(const std::string_view start,
					       const std::string_view end,
					       std::string_view& region) {
  if (auto spos = region.find(start);
      spos != std::string_view::npos) {
    if (auto epos = region.find(end, spos + start.length());
	epos != std::string_view::npos) {
      std::string_view matched = region.substr(spos, epos - spos);
      auto consumed = matched.size() + end.length();
  ...
  AWSv4ComplMulti::trailer_map_t& trailer_map)
{
  ...
  for (auto k = kv.begin(); k != kv.end(); k = std::next(k)) {
    ...
    auto ex_header = extract_helper(mut_sv_trailer, *k, "\r\n");
    if (get<0>(ex_header)) {
