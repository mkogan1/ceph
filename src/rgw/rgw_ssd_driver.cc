...
int SSDDriver::ensure_thread_uring(const DoutPrefixProvider* dpp, struct io_uring** ring_out) const
{
    ...
    if (dpp->get_cct()->_conf->rgw_d4n_io_uring_iopoll) {
        ...
        ldpp_dout(dpp, 10) << "SSDDriver: io_uring initialized with IOPOLL mode" << dendl;
    }
    if (dpp->get_cct()->_conf->rgw_d4n_io_uring_sqpoll) {
        flags |= IORING_SETUP_SQPOLL;
        ldpp_dout(dpp, 10) << "SSDDriver: io_uring initialized with SQPOLL mode" << dendl;
    }
...
int rgw::cache::SSDDriver::AsyncWriteRequest::prepare_io_uring_write_op(const DoutPrefixProvider *dpp, bufferlist& bl, unsigned int len, std::string file_path, struct io_uring* ring)
{
    ...
    data = buffer_pool ? buffer_pool->allocate(len) : ::malloc(len);
    if (!data) {
    ...
    struct io_uring* ring)
{
    ...
    buffer = buffer_pool ? buffer_pool->allocate(read_len) : ::malloc(read_len);
    if (!buffer) {
