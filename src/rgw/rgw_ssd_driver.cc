...
int SSDDriver::initialize(const DoutPrefixProvider* dpp)
{
    ...
    // Configure buffer pool with the configured max buffers per size
    buffer_pool_.set_max_buffers_per_size(dpp->get_cct()->_conf->rgw_d4n_io_uring_buffer_pool_size);
...
#endif
    if (g_conf().get_val<bool>("rgw_d4n_io_uring_direct_io")) {
...
int rgw::cache::SSDDriver::AsyncWriteRequest::prepare_io_uring_write_op(const DoutPrefixProvider *dpp, bufferlist& bl, unsigned int len, std::string file_path, struct io_uring* ring)
{
    ...
    r = fd = TEMP_FAILURE_RETRY(::open(file_path.c_str(), open_flags, mode));
    if (fd < 0) {
    ...
    struct io_uring* ring)
{
    ...
    // For O_DIRECT, we need aligned buffers - buffer_pool now provides aligned memory
