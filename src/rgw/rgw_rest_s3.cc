...
struct ReplicationConfiguration {
  ...
  struct Rule {
    ...
    set<rgw_zone_id> get_zone_ids_from_names(RGWRados *store,
    ...
    vector<string> get_zone_names_from_ids(RGWRados *store,
    ...
    int to_sync_policy_pipe(req_state *s, RGWRados *store,
      ...
      const auto& user_id = s->user->user_id;

      rgw_bucket dest_bk;
      dest_bk.name = destination.bucket;
      dest_bk.tenant = user_id.tenant;
    ...
    void from_sync_policy_pipe(RGWRados *store,
  ...
  int to_sync_policy_groups(req_state *s, RGWRados *store,
  ...
  void from_sync_policy_group(RGWRados *store,
...
int RGWHandler_REST_S3::postauth_init()
{
  ...
  if (!t->src_bucket.empty()) {
    ...
    if (s->auth.identity->get_identity_type() == TYPE_ROLE) {
      ...
      auth_tenant = s->user->user_id.tenant;
    }
