...
struct Condition {
  ...
  using unordered_multimap_it_pair = std::pair <std::unordered_multimap<string,string>::const_iterator, std::unordered_multimap<string,string>::const_iterator>;
...
struct Policy {
  ...
  template <typename F>
  bool has_conditional_value(const string& conditional, F p) const {
    for (const auto&s: statements){
      if (std::any_of(s.conditions.begin(), s.conditions.end(),
  ...
  bool has_partial_conditional_value(const string& c) const {
