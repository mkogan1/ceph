...
struct lc_op_ctx {
  ...
  lc_op_ctx(op_env& _env, rgw_bucket_dir_entry& _o,
    ...
    : cct(_env.store->ctx()), env(_env), o(_o), store(env.store),
      bucket_info(env.bucket_info), op(env.op), ol(env.ol),
...
public:
  WorkQ(RGWLC::LCWorker* wk, uint32_t ix, uint32_t qmax, work_f f)
    : wk(wk), qmax(qmax), f(f)
    {
...
class RGWLC::WorkPool
{
  ...
  WorkQ::work_f bsf = [](RGWLC::LCWorker* wk, WorkItem& wi) {};
...
public:
  WorkPool(RGWLC::LCWorker* wk, uint16_t n_threads, uint32_t qmax)
	...
	[&](const size_t ix, auto emplacer) {
	  emplacer.emplace(wk, ix, qmax, bsf);
	}}),
      ix(0)
    {}
  ...
  LCWorker* worker)
{
  ...
  for (auto prefix_iter = prefix_map.begin(); prefix_iter != prefix_map.end();
	...
	if (going_down()) {
      ...
      /* permit pasing o by reference, removes fetch overlap */
      worker->workpool->drain();
      std::this_thread::sleep_for(std::chrono::milliseconds(delay_ms));
