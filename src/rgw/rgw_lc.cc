...
void *RGWLC::LCWorker::entry() {
  do {
    rgw_bucket* all_buckets = nullptr; // empty restriction
...
struct op_env {
  ...
  op_env(lc_op& _op, RGWRados *_store, LCWorker* _worker,
    ...
    : op(_op), store(_store), worker(_worker),
      bucket_info(_bucket_info), ol(_ol) {}
}; /* op_env */
...
class LCOpRule {
  ...
  std::vector<shared_ptr<LCOpFilter> > filters; // XXXX shared
  std::vector<shared_ptr<LCOpAction> > actions; // XXXX shared
  ...
  int process(lc_op_ctx& oc) {
	...
	ldout(oc.cct, 0) << "ERROR: current is-dm remove_expired_obj "
      ...
      ldout(oc.cct, 2) << "DELETED: current is-dm "
		       << oc.bucket_info.bucket << ":" << o.key
	//<< " next key: " << oc.op.get_next_key_name()
		       << " " << oc.wq->thr_name() << dendl;
    } else {
      /* ! o.is_delete_marker() */
...
public:
  ...
  bool check(lc_op_ctx& oc, ceph::real_time *exp_time) override {
    ...
    return is_expired &&
      pass_object_lock_check(oc.store, oc.bucket_info, oc.obj, oc.rctx);
  }
			  ...
			  LCWorker* worker)
{
  ...
  do {
    int ret = l.lock_exclusive(
      &store->lc_pool_ctx, obj_names[index]);
			  ...
			  bool once = false)
{
  ...
  l.set_duration(time);
  ret = l.lock_exclusive(&store->lc_pool_ctx, obj_names[index]);
  if (ret == -EBUSY || ret == -EEXIST) {
    /* already locked by another lc processor */
  ...
  if (! (cct->_conf->rgw_lc_lock_max_time == 9969)) {
    ret = cls_rgw_lc_get_entry(store->lc_pool_ctx, obj_names[index], bucket_entry_marker, entry);
    if (ret >= 0) {
...
{
  int ret{0};
  cls_rgw_lc_entry next_entry;

  ret = cls_rgw_lc_get_next_entry(store->lc_pool_ctx, lc_shard, entry.bucket,
				  next_entry);
  if (ret < 0) {
		   ...
		   bool once = false)
{
  ...
  cls_rgw_lc_obj_head head;
  cls_rgw_lc_entry entry; //string = bucket_name:bucket_id, start_time, int = LC_BUCKET_STATUS
  ...
  do {
	...
	head.shard_rollover_date = 0;
      ...
      if (ret == -ENOENT) {
	ret = cls_rgw_lc_get_next_entry(store->lc_pool_ctx, lc_shard, head.marker, entry);
        if (ret < 0) {
    ...
    if (ret == -ENOENT) {
       ...
       * there is no such bucket?  It is currently the value returned
       * from bucket_lc_process(...) */
      ret = cls_rgw_lc_rm_entry(store->lc_pool_ctx, lc_shard,  entry);
      if (ret < 0) {
