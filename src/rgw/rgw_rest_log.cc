...
void RGWOp_BILog_List::execute() {
  ...
  do {
    ...
    int ret = store->list_bi_log_entries(bucket_info, log_layout, shard_id,
...
void RGWOp_BILog_Info::execute() {
  ...
  int ret =  store->get_bucket_stats(bucket_info, index, shard_id, &bucket_ver, &master_ver, stats, &max_marker, &syncstopped);
  if (ret < 0 && ret != -ENOENT) {
    http_ret = ret;
    return;
  }
...
void RGWOp_DATALog_Notify2::execute() {
  string  source_zone = s->info.args.get("source-zone");
#define LARGE_ENOUGH_BUF (128 * 1024)
...
void RGWOp_BILog_Status::execute()
{
  ...
  http_ret = store->get_sync_policy_handler(source_zone_id, source_bucket, &source_handler);
  if (http_ret < 0) {
  ...
  for (auto& entry : local_dests) {
    ...
    if (*pipe.dest.bucket != info.bucket) {
      ...
      http_ret = store->get_bucket_info(obj_ctx,
					pipe.dest.bucket->tenant,
					pipe.dest.bucket->name,
					*pinfo, nullptr);
      if (http_ret < 0) {
