...
void RGWOp_BILog_List::execute(optional_yield y) {
  ...
  do {
    ...
    int ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->log_list(s, bucket_info, log_layout, shard_id,
...
void RGWOp_BILog_Delete::execute(optional_yield y) {
  ...
  op_ret = bilog_trim(this, static_cast<rgw::sal::RGWRadosStore*>(store),
		      bucket_info, gen, shard_id,
...
#define LARGE_ENOUGH_BUF (128 * 1024)
  ...
  store->getRados()->wakeup_data_sync_shards(this, source_zone, updated_shards);
...
#define LARGE_ENOUGH_BUF (128 * 1024)
  ...
  store->getRados()->wakeup_data_sync_shards(s, source_zone, updated_shards);
...
void RGWOp_BILog_Status::execute(optional_yield y)
{
  ...
  if (!merge) {
    ...
    op_ret = rgw_read_bucket_full_sync_status(
      this,
      static_cast<rgw::sal::RGWRadosStore*>(store),
      pipe,
      &status.sync_status,
      s->yield);
    if (op_ret < 0) {
