...
int RGWCuObjServer::do_init(CephContext* cct)
{
  ...
  if (rdma_ip.empty()) {
    lderr(cct) << "rgw_cuobj: rgw_cuobj_rdma_ip not configured" << dendl;
    return -EINVAL;
  }

  auto rdma_port = static_cast<unsigned short>(
      cct->_conf.get_val<uint64_t>("rgw_cuobj_rdma_port"));
  auto num_dcis = static_cast<int>(
      cct->_conf.get_val<uint64_t>("rgw_cuobj_num_dcis"));
  auto buf_size = static_cast<size_t>(
      cct->_conf.get_val<Option::size_t>("rgw_cuobj_buffer_size"));
  auto buf_count = static_cast<size_t>(
      cct->_conf.get_val<uint64_t>("rgw_cuobj_buffer_count"));
  ...
  try {
    ...
    lderr(cct) << "rgw_cuobj: cuObjServer construction failed: "
               << e.what() << dendl;
    return -EIO;
  }
  ...
  if (!m_server->isConnected()) {
    lderr(cct) << "rgw_cuobj: cuObjServer failed to connect" << dendl;
    m_server.reset();
    return -ECONNREFUSED;
  }
  ...
  for (size_t i = 0; i < buf_count; i++) {
    ...
    if (!entry.ptr) {
      lderr(cct) << "rgw_cuobj: allocHostBuffer failed for buffer " << i << dendl;
      do_shutdown();
      return -ENOMEM;
    }
    entry.size = buf_size;
    ...
    if (!entry.handle) {
      lderr(cct) << "rgw_cuobj: registerBuffer failed for buffer " << i << dendl;
      free(entry.ptr);
      entry.ptr = nullptr;
      do_shutdown();
      return -EIO;
...
size_t RGWCuObjServer::parse_rdma_descriptor_size(const std::string& rdma_descr)
{
  ...
  auto size_str = rdma_descr.substr(first_colon + 1,
...
RGWCuObjServer::RDMABufEntry* RGWCuObjServer::acquire_buffer(size_t needed_size)
{
  ...
  ldout(m_cct, 5) << "rgw_cuobj: no available RDMA buffer for size "
                   << needed_size << dendl;
  return nullptr;
}
    ...
    const std::string& rdma_descr)
{
  ...
  ldout(m_cct, 1) << "rgw_cuobj: handlePutObject"
                   << " key=" << key
                   << " size=" << size
                   << " client_addr=0x" << std::hex << client_addr << std::dec
                   << " channel=" << channel
                   << " buf_ptr=" << buf->ptr
                   << " buf_size=" << buf->size
                   << " descr=" << rdma_descr << dendl;
  ...
  while (total_read < size) {
    ...
    if (ret < 0) {
      lderr(m_cct) << "rgw_cuobj: handlePutObject failed:"
                   << " ret=" << ret
                   << " wc_status=" << static_cast<int>(wc_status)
                   << " chunk=" << chunk
                   << " local_offset=" << total_read
                   << " client_addr=0x" << std::hex << (client_addr + total_read) << std::dec
                   << dendl;
      return ret;
    }
    total_read += ret;
  ...
  ldout(m_cct, 1) << "rgw_cuobj: RDMA read " << total_read
                   << " bytes for key=" << key << dendl;
    ...
    const std::string& rdma_descr)
{
  ...
  while (total_written < size) {
    ...
    if (ret < 0) {
      lderr(m_cct) << "rgw_cuobj: handleGetObject failed: " << ret << dendl;
      return ret;
    }
    total_written += ret;
    if (static_cast<size_t>(ret) < chunk) {
  ...
  ldout(m_cct, 1) << "rgw_cuobj: RDMA wrote " << total_written
                    << " bytes for key=" << key << dendl;
