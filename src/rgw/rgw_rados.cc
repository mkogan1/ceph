...
{
  int r = do_complete(etag, mtime, set_mtime, attrs, delete_at, if_match, if_nomatch, user_data, zones_trace);
  if (r < 0)
    return r;
...
public:
  ...
  int complete(string& etag, real_time *mtime, real_time set_mtime, map<string, bufferlist>& attrs, real_time delete_at, rgw_zone_set *zones_trace) {
    return processor->complete(etag, mtime, set_mtime, attrs, delete_at,
                               nullptr, nullptr, nullptr, zones_trace);
  }
...
int RGWRados::Bucket::UpdateIndex::prepare(RGWModifyOp op)
{
  ...
  ret = store->cls_obj_prepare_op(*bs, op, optag, obj, bilog_flags, zones_trace);
...
int RGWRados::bucket_index_unlink_instance(rgw_obj& obj_instance, const string& op_tag, const string& olh_tag, uint64_t olh_epoch, rgw_zone_set *_zones_trace)
{
  rgw_rados_ref ref;
  rgw_bucket bucket;
  int r = get_obj_ref(obj_instance, &ref, &bucket);
 ...
 */
int RGWRados::update_olh(RGWObjectCtx& obj_ctx, RGWObjState *state, RGWBucketInfo& bucket_info, rgw_obj& obj, rgw_zone_set *zones_trace)
{
  map<uint64_t, vector<rgw_bucket_olh_log_entry> > log;
  bool is_truncated;
...
#define MAX_ECANCELED_RETRY 100
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    ret = bucket_index_link_olh(*state, target_obj, delete_marker, op_tag, meta, olh_epoch, unmod_since, high_precision_time, zones_trace, log_data_change);
    if (ret < 0) {
                                  ...
                                  uint64_t olh_epoch, rgw_zone_set *zones_trace)
{
  ...
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    ret = bucket_index_unlink_instance(target_obj, op_tag, olh_tag, olh_epoch, zones_trace);
    if (ret < 0) {
...
int RGWRados::get_bucket_instance_info(RGWObjectCtx& obj_ctx, const string& meta_key, RGWBucketInfo& info, ceph::real_time *pmtime, std::map<string, bufferlist> *pattrs)
{
...
int RGWRados::get_bucket_instance_info(RGWObjectCtx& obj_ctx, const rgw_bucket& bucket, RGWBucketInfo& info, ceph::real_time *pmtime, std::map<string, bufferlist> *pattrs)
{
  string oid;
  if (bucket.oid.empty()) {
    get_bucket_meta_oid(bucket, oid);
...
int RGWRados::get_bucket_instance_from_oid(RGWObjectCtx& obj_ctx, const std::string& oid, RGWBucketInfo& info, ceph::real_time *pmtime, std::map<string, bufferlist> *pattrs, rgw_cache_entry_info *cache_info, boost::optional<obj_version> refresh_version)
{
                                 ...
                                 rgw_obj& obj, uint16_t bilog_flags, rgw_zone_set *_zones_trace)
{
  ...
  cls_rgw_obj_key key(obj.get_index_key_name(), obj.get_instance());
  cls_rgw_bucket_prepare_op(o, op, tag, key, obj.get_loc(), get_zone().log_data, bilog_flags, zones_trace);
...
int RGWRados::cls_obj_complete_op(BucketShard& bs, RGWModifyOp op, string& tag,
				  ...
				  list<rgw_obj_key> *remove_objs, uint16_t bilog_flags, rgw_zone_set *_zones_trace)
{
...
int RGWRados::cls_obj_complete_add(BucketShard& bs, string& tag,
                                   int64_t pool, uint64_t epoch,
                                   RGWObjEnt& ent, RGWObjCategory category,
                                   list<rgw_obj_key> *remove_objs, uint16_t bilog_flags, rgw_zone_set *zones_trace)
{
...
int RGWRados::cls_bucket_head(rgw_bucket& bucket, int shard_id, vector<rgw_bucket_dir_header>& headers, map<int, string> *bucket_instance_ids)
{
  librados::IoCtx index_ctx;
  map<int, string> oids;
  map<int, struct rgw_cls_list_ret> list_results;
...
int RGWRados::cls_user_sync_bucket_stats(rgw_obj& user_obj, rgw_bucket& bucket)
{
  ...
  for (const auto& hiter : headers) {
    for (const auto& iter : hiter.stats) {
      const rgw_bucket_category_stats& header_stats = iter.second;
