  ...
  for (uint16_t attempt = 1; /* empty */; ++attempt) {
	...
	ldpp_dout(dpp, 20) << __PRETTY_FUNCTION__ <<
      ...
      if (cur_end_marker_valid && cur_end_marker <= index_key) {
        truncated = false;
	ldpp_dout(dpp, 10) << __PRETTY_FUNCTION__ <<
	  ": finished due to hitting end marker of \"" << cur_end_marker <<
...
int RGWRados::reindex_obj(RGWBucketInfo& bucket_info,
               ...
               std::optional<uint64_t>* bytes_transferred)
{
  ...
  obj_time_weight dest_mtime_weight;
  rgw_zone_set_entry dst_zone_trace(svc.zone->get_zone().id, dest_bucket->get_info().bucket.get_key());

  if (copy_if_newer) {
    /* need to get mtime for destination */
    ret = get_obj_state(dpp, &obj_ctx, dest_bucket->get_info(), stat_dest_obj, &dest_state, stat_follow_olh, null_yield);
    if (ret < 0)
      goto set_err_state;
               ...
               optional_yield y)
{
  ...
  rgw_obj stat_dest_obj = dest_obj->get_obj();
...
int RGWRados::set_attrs(const DoutPrefixProvider *dpp, void *ctx, RGWBucketInfo& bucket_info, const rgw_obj& src_obj,
                        map<string, bufferlist>& attrs,
...
int RGWRados::bucket_index_clear_olh(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info, const std::string& olh_tag, const rgw_obj& obj_instance)
{
  rgw_rados_ref ref;
  int r = get_obj_head_ref(dpp, bucket_info, obj_instance, &ref);
  if (r < 0) {
			    ...
			    rgw_zone_set* zones_trace)
{
  ...
  *plast_ver = last_ver;
  ...
  for (list<cls_rgw_obj_key>::iterator liter = remove_instances.begin();
    ...
    int ret = delete_obj(dpp, obj_ctx, bucket_info, obj_instance, 0, null_verid, RGW_BILOG_FLAG_VERSIONED_OP, ceph::real_time(), zones_trace);
    if (ret < 0 && ret != -ENOENT) {
 ...
 */
int RGWRados::update_olh(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWObjState *state, RGWBucketInfo& bucket_info, const rgw_obj& obj, rgw_zone_set *zones_trace, bool null_verid)
{
  ...
  do {
    ...
    ret = apply_olh_log(dpp, obj_ctx, *state, bucket_info, obj, state->olh_tag, log, &ver_marker, null_verid, zones_trace);
    if (ret < 0) {
      return ret;
    }
  } while (is_truncated);
...
int RGWRados::set_olh(const DoutPrefixProvider *dpp, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& target_obj, bool delete_marker, rgw_bucket_dir_entry_meta *meta,
                                  ...
                                  uint64_t olh_epoch, optional_yield y,  uint16_t bilog_flags, bool null_verid, rgw_zone_set *zones_trace)
{
  ...
  for (i = 0; i < MAX_ECANCELED_RETRY; i++) {
    ...
    if (ret < 0) {
      ...
      // object from being cleaned by another thread that was deleting the last
      // existing version. We invoke a best-effort update_olh here to handle this case.
      int r = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, zones_trace, null_verid);
      if (r < 0 && r != -ECANCELED) {
        ldpp_dout(dpp, 20) << "update_olh() target_obj=" << olh_obj << " returned " << r << dendl;
      }
      return ret;
  ...
  ret = update_olh(dpp, obj_ctx, state, bucket_info, olh_obj, zones_trace, null_verid);
  if (ret == -ECANCELED) { /* already did what we needed, no need to retry, raced with another user */
    return 0;
  }
  if (ret < 0) {
                                 ...
                                 rgw_obj& obj, uint16_t bilog_flags, optional_yield y, rgw_zone_set *_zones_trace)
{
  ...
  ldout_bitx(bitx, cct, 10) << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(0) << dendl_bitx;
				      ...
				      RGWBucketListNameFilter force_check_filter)
{
  ...
  ldout_bitx(bitx, cct, 20) << "BACKTRACE: " << __func__ << ": " << ClibBackTrace(0) << dendl_bitx;
