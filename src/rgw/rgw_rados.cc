...
void RGWRados::wakeup_data_sync_shards(const rgw_zone_id& source_zone, bc::flat_map<int, bc::flat_set<rgw_data_notify_entry> >& entries)
{
  ldout(cct, 20) << __func__ << ": source_zone=" << source_zone << ", entries=" << entries << dendl;
  for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry> >::iterator iter = entries.begin(); iter != entries.end(); ++iter) {
    ldout(cct, 20) << __func__ << "(): updated shard=" << iter->first << dendl;
    bc::flat_set<rgw_data_notify_entry>& entries = iter->second;
    for (const auto& [key, gen] : entries) {
      ldout(cct, 20) << __func__ << ": source_zone=" << source_zone << ", key=" << key
			 << ", gen=" << gen << dendl;
    }
  }
...
void add_datalog_entry(CephContext* cct,
                       ...
                       uint32_t shard_id)
{
  ...
  int r = datalog->add_entry(bucket_info, logs.back(), shard_id);
  if (r < 0) {
    ldout(cct, -1) << "ERROR: failed writing data log" << dendl;
  ...
  while(!_stop) {
    ...
    for (auto c : comps) {
      ...
      r = store->guard_reshard(&bs, c->obj, bucket_info,
				 ...
				 "BACKTRACE: " << __func__ << ": " << BackTrace(1) << dendl_bitx;
				 ...
				 const rgw::bucket_index_layout_generation& idx_layout)
{
  ...
  get_bucket_index_objects(dir_oid,
			   idx_layout.layout.normal.num_shards,
			    ...
			    bool exclusive)
{
  ...
  for (int i = 0; i < MAX_CREATE_RETRIES; i++) {
	...
	int r = clean_bucket_index(info, info.layout.current_index);
        if (r < 0) {
 ...
 * up without one
 */
int RGWRados::fix_head_obj_locator(RGWBucketInfo& bucket_info, bool copy_obj, bool remove_bad, rgw_obj_key& key)
{
 ...
 * up without one
 */
int RGWRados::fix_tail_obj_locator(RGWBucketInfo& bucket_info, rgw_obj_key& key, bool fix, bool *need_fix)
{
  const rgw_bucket& bucket = bucket_info.bucket;
  rgw_obj obj(bucket, key);
               ...
               std::optional<uint64_t>* bytes_transferred)
{
  ...
  if (override_owner) {
    ...
    if (rgw_get_user_info_by_uid(this, *override_owner, owner_info) < 0) {
...
{
  int ret = open_bucket_index(bucket_info, index_ctx, idx_layout, oids, shard_id, bucket_instance_ids);
  if (ret < 0)
    return ret;
				      ...
				      int *shard_id)
{
  ...
  ret = get_bucket_index_object(bucket_oid_base, obj_key,
				...
				bucket_info.layout.current_index.gen,
				bucket_obj, shard_id);
  if (ret < 0) {
...
int RGWRados::olh_init_modification(RGWBucketInfo& bucket_info, RGWObjState& state, const rgw_obj& obj, string *op_tag)
{
  int ret;
...
int RGWRados::get_olh(RGWBucketInfo& bucket_info, const rgw_obj& obj, RGWOLHInfo *olh)
{
  map<string, bufferlist> attrset;
...
int RGWRados::remove_olh_pending_entries(RGWBucketInfo& bucket_info, RGWObjState& state, const rgw_obj& olh_obj, map<string, bufferlist>& pending_attrs)
{
  rgw_rados_ref ref;
...
int RGWRados::get_bucket_stats(RGWBucketInfo& bucket_info,
			       const rgw::bucket_index_layout_generation& idx_layout,
			       ...
			       map<RGWObjCategory, RGWStorageStats>& stats, string *max_marker, bool *syncstopped)
{
  ...
  map<int, string> bucket_instance_ids;
  int r = cls_bucket_head(bucket_info, idx_layout, shard_id, headers, &bucket_instance_ids);
  if (r < 0) {
    return r;
  }
...
int RGWRados::get_bi_log_status(RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id,
    map<int, string>& markers)
{
  vector<rgw_bucket_dir_header> headers;
  map<int, string> bucket_instance_ids;
  const auto& current_index = rgw::log_to_index_layout(log_layout);
  int r = cls_bucket_head(bucket_info, current_index, shard_id, headers, &bucket_instance_ids);
  if (r < 0)
    return r;
...
{
  int num_aio = 0;
  RGWGetBucketStatsContext *get_ctx = new RGWGetBucketStatsContext(ctx, bucket_info.layout.current_index.layout.normal.num_shards ?  bucket_info.layout.current_index.layout.normal.num_shards : 1);
  ceph_assert(get_ctx);
  int r = cls_bucket_head_async(bucket_info, idx_layout,
				shard_id, get_ctx, &num_aio);
  if (r < 0) {
    ctx->put();
    if (num_aio) {
      get_ctx->unset_cb();
...
int RGWRados::update_containers_stats(map<string, RGWBucketEnt>& m)
{
  ...
  for (iter = m.begin(); iter != m.end(); ++iter) {
    ...
    int r = cls_bucket_head(bucket_info, bucket_info.layout.current_index, RGW_NO_SHARD, headers);
    if (r < 0)
      return r;
...
int RGWRados::list_bi_log_entries(RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id, string& marker, uint32_t max,
                                  std::list<rgw_bi_log_entry>& result, bool *truncated)
{
  ...
  const auto& current_index = rgw::log_to_index_layout(log_layout);
  int r = open_bucket_index(bucket_info, index_ctx, current_index, oids, shard_id);
  if (r < 0)
    return r;
...
int RGWRados::trim_bi_log_entries(RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id, std::string_view start_marker, std::string_view end_marker)
{
  ...
  const auto& current_index = rgw::log_to_index_layout(log_layout);
  int r = open_bucket_index(bucket_info, index_ctx, current_index, bucket_objs, shard_id);
  if (r < 0) {
    return r;
  }
...
int RGWRados::resync_bi_log_entries(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id)
{
  librados::IoCtx index_ctx;
  map<int, string> bucket_objs;
  const auto& current_index = rgw::log_to_index_layout(log_layout);
...
int RGWRados::stop_bi_log_entries(const RGWBucketInfo& bucket_info, const rgw::bucket_log_layout_generation& log_layout, int shard_id)
{
  librados::IoCtx index_ctx;
  map<int, string> bucket_objs;
  const auto& current_index = rgw::log_to_index_layout(log_layout);
...
int RGWRados::bi_list(rgw_bucket& bucket, const string& obj_name_filter, const string& marker, uint32_t max, list<rgw_cls_bi_entry> *entries, bool *is_truncated)
{
  ...
  ret = cls_rgw_bi_list(bs.index_ctx, bs.bucket_obj, obj_name_filter, marker, max, entries, is_truncated);
  if (ret == -ENOENT) {
    *is_truncated = false;
  }
  if (ret < 0)
...
{
  int ret = cls_rgw_bi_list(bs.index_ctx, bs.bucket_obj, obj_name_filter, marker, max, entries, is_truncated);
  if (ret < 0)
    return ret;
...
int RGWRados::cls_obj_set_bucket_tag_timeout(RGWBucketInfo& bucket_info,
					     uint64_t timeout)
{
  librados::IoCtx index_ctx;
  map<int, string> bucket_objs;
				      ...
				      bool (*force_check_filter)(const string& name))
{
  ...
  ldout_bitx(bitx, cct, 10) << "ENTERING " << __func__ << ": " << bucket_info.bucket <<
    ...
    ", expansion_factor=" << expansion_factor << dendl_bitx;
 ...
 ldout_bitx(bitx, cct, 20) << "EXITING " << __func__ << dendl_bitx;
					...
					bool (*force_check_filter)(const string& name))
{
  ...
  ldout_bitx(bitx, cct, 10) << "ENTERING " << __func__ << ": " << bucket_info.bucket <<
    ...
    "\", prefix=\"" << prefix <<
    "\", num_entries=" << num_entries <<
    ", list_versions=" << list_versions << dendl_bitx;
  ldout_bitx(bitx, cct, 25) << "BACKTRACE: " << __func__ << ": " << BackTrace(1) << dendl_bitx;
				     ...
				     const std::list<rgw_obj_index_key>& entry_key_list)
{
  ...
  int r = open_bucket_index(bucket_info, index_ctx, current_index, index_oids);
  if (r < 0) {
    return r;
  }
  ...
  ldout_bitx(bitx, cct, 10) << "INFO: " << __func__ <<
    ...
    sharded_updates.size() << " of " << index_oids.size() << " shards" << dendl_bitx;
                               ...
                               bufferlist& suggested_updates)
{
  ...
  if (!astate->exists) {
    ...
    if (list_state.exists) {
      ldout_bitx(bitx, cct, 10) <<
	"INFO: " << __func__ << ": index list state exists" << dendl_bitx;
    ...
    ldout_bitx(bitx, cct, 10) <<
      ...
      list_state.key << " on suggested_updates" << dendl_bitx;
  ...
  if (astate->has_manifest) {
	...
	ldout_bitx(bitx, cct, 10) <<
	  "INFO: " << __func__ << ": removing manifest part from index loc=" << loc << dendl_bitx;
...
int RGWRados::cls_bucket_head(const RGWBucketInfo& bucket_info,
			      const rgw::bucket_index_layout_generation& idx_layout,
			      int shard_id, vector<rgw_bucket_dir_header>& headers, map<int, string> *bucket_instance_ids)
{
  librados::IoCtx index_ctx;
  map<int, string> oids;
  map<int, struct rgw_cls_list_ret> list_results;
...
int RGWRados::cls_bucket_head_async(const RGWBucketInfo& bucket_info,
				    const rgw::bucket_index_layout_generation& idx_layout,
				    int shard_id, RGWGetDirHeader_CB *ctx, int *num_aio)
{
  librados::IoCtx index_ctx;
  map<int, string> bucket_objs;
  int r = open_bucket_index(bucket_info, index_ctx, idx_layout, bucket_objs, shard_id);
  if (r < 0)
    return r;
...
int RGWRados::cls_user_get_bucket_stats(const rgw_bucket& bucket, cls_user_bucket_entry& entry)
{
  ...
  ret = cls_bucket_head(bucket_info, bucket_info.layout.current_index, RGW_NO_SHARD, headers);
  if (ret < 0) {
				  ...
				  uint64_t num_objs)
{
  ...
  quota_handler->check_bucket_shards(max_objs_per_shard, num_source_shards,
                                     bucket, num_objs, is_multisite, need_resharding,
  ...
  ldout(cct, 5) << "INFO: RGWRados::" << __func__ << ": bucket=" << bucket.name <<
...
RGWRados *RGWStoreManager::init_storage_provider(CephContext *cct, bool use_gc_thread, bool use_lc_thread,
						 bool quota_threads, bool run_sync_thread, bool run_reshard_thread, bool use_cache, bool use_gc)
{
...
int RGWRados::handle_overwrite(const RGWBucketInfo& info,
  ...
  if (!new_sync_enabled) {
    ret = stop_bi_log_entries(info, bilog, -1);
  } else {
    ret = resync_bi_log_entries(info, bilog, -1);
  }
  if (ret < 0) {
    ldout(cct, -1) << "ERROR: failed writing bilog (bucket=" << info.bucket << "); ret=" << ret << dendl;
    return ret;
  }
