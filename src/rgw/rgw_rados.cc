...
{
  ldout(ctx(), 20) << __func__ << ": source_zone=" << source_zone << ", entries=" << entries << dendl;
  for (bc::flat_map<int, bc::flat_set<rgw_data_notify_entry> >::iterator iter = entries.begin(); iter != entries.end(); ++iter) {
    ldout(ctx(), 20) << __func__ << "(): updated shard=" << iter->first << dendl;
    bc::flat_set<rgw_data_notify_entry>& entries = iter->second;
    for (const auto& [key, gen] : entries) {
      ldout(ctx(), 20) << __func__ << ": source_zone=" << source_zone << ", key=" << key
			 << ", gen=" << gen << dendl;
    }
  }
  ...
  for (auto c : comps) {
    ...
    r = store->guard_reshard(this, &bs, c->obj, bucket_info,
			     [&](RGWRados::BucketShard *bs) -> int {
			       ...
			       ldout_bitx(bitx, cct, 10) << "ENTERING " << __func__ << ": bucket-shard=" << bs << " obj=" << c->obj << " tag=" << c->tag << " op=" << c->op << ", remove_objs=" << c->remove_objs << dendl_bitx;
			       ...
			       int ret = bs->bucket_obj.operate(this, &o, null_yield);
    ...
    r = store->svc.datalog_rados->add_entry(this,
					    bucket_info,
...
int RGWRados::decode_policy(ceph::buffer::list& bl,
			    ACLOwner *owner)
{
  auto i = bl.cbegin();
...
int RGWRados::Object::Delete::delete_obj(optional_yield y, const DoutPrefixProvider *dpp)
{
  ...
  if (params.versioning_status & BUCKET_VERSIONED || explicit_marker_version) {
    ...
    r = store->svc.datalog_rados->add_entry(dpp,
					    target->bucket_info,
                                            ...
                                            bool appendable)
{
  ...
  int r = store->svc.datalog_rados->add_entry(dpp,
					      target->bucket_info,
					      target->bucket_info.layout.logs.back(),
					      bs->shard_id);
  if (r < 0) {
                                     ...
                                     const DoutPrefixProvider *dpp)
{
  ...
  for (int i = 1; i <= num_retries; i++) { // nb: 1-based for loop
	...
	ret = RGWBucketReshard::clear_resharding(dpp, this->store, bucket_info, bucket_attrs);
	if (ret < 0) {
	  reshard_lock.unlock();
	  ldpp_dout(dpp, 0) << __func__ <<
                                    ...
                                    rgw_zone_set *_zones_trace, bool log_data_change)
{
  ...
  r = svc.datalog_rados->add_entry(dpp,
				   bucket_info,
				   bucket_info.layout.logs.back(),
				   bs.shard_id);
  if (r < 0) {
...
int RGWRados::bucket_index_unlink_instance(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info,
                                           const rgw_obj& obj_instance,
...
int RGWRados::bucket_index_trim_olh_log(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info,
                                        RGWObjState& state,
...
int RGWRados::bucket_index_clear_olh(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info,
                                     RGWObjState& state,
                                     const rgw_obj& obj_instance)
{
  rgw_rados_ref ref;
...
int RGWRados::get_bucket_stats(const DoutPrefixProvider *dpp, RGWBucketInfo& bucket_info,
...
int RGWRados::bi_list(const DoutPrefixProvider *dpp, rgw_bucket& bucket, const string& obj_name_filter, const string& marker, uint32_t max, list<rgw_cls_bi_entry> *entries, bool *is_truncated)
{
				     ...
				     const std::list<rgw_obj_index_key>& entry_key_list)
{
  ...
  r = CLSRGWIssueBucketBIDirSuggest(index_pool.ioctx(),
				    index_oids,
				    sharded_updates,
				    cct->_conf->rgw_bucket_index_max_aio)();
...
int RGWRados::cls_bucket_head_async(const DoutPrefixProvider *dpp, const RGWBucketInfo& bucket_info, const rgw::bucket_index_layout_generation& idx_layout, int shard_id, RGWGetDirHeader_CB *ctx, int *num_aio)
{
  ...
  int r = svc.bi_rados->open_bucket_index(
    dpp, bucket_info, shard_id, idx_layout, &index_pool, &bucket_objs, nullptr);
  if (r < 0)
    return r;
                                  ...
                                  const DoutPrefixProvider *dpp)
{
  ...
  ldpp_dout(dpp, 5) << "INFO: RGWRados::" << __func__ << ": bucket=" << bucket.name <<
