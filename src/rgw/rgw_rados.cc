  ...
  optional_yield y)
{
  ...
  for (uint16_t attempt = 1; /* empty */; ++attempt) {
	...
	ldpp_dout(dpp, 20) << __func__ <<
	  ...
	  "\"" << dendl;
>>>>>>> 91488321277 (rgw: optimize bucket listing to skip past regions of namespaced entries):src/rgw/driver/rados/rgw_rados.cc
        continue;
      }
			  ...
			  optional_yield y)
{
  ...
  std::map<std::string, bufferlist> attr_set = head_state->attrset;
...
int RGWRados::get_bucket_stats(const DoutPrefixProvider *dpp,
			       ...
			       int shard_id, string *bucket_ver,
			       string *master_ver,
			       map<RGWObjCategory, RGWStorageStats>& stats,
