...
int RGWRestore::process_restore_entry(RGWRestoreEntry& entry, optional_yield y)
{
  ...
  ret = obj->get_obj_state(this, &obj_state, null_yield, true);

  if (ret < 0) {
    ldpp_dout(this, -1) << "ERROR: Restore:get_object for " << entry.obj_key
		       << " failed" << dendl;
    return ret;
  }
				       ...
				       optional_yield y)
{
  ...
  if (ret < 0) {
    ldpp_dout(this, -1) << "ERROR: object " << pobj->get_key() << " fetching failed" << ret << dendl;
