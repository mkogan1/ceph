                       ...
                       bufferlist& in_data, JSONParser& parser)
{
  ...
  param_vec_t params;
  RGWRESTSimpleRequest req(g_ceph_context, info.method, url, NULL, &params, __PRETTY_FUNCTION__);
...
static void get_md_sync_status(list<string>& status)
{
  ...
  if (sync_status.sync_info.period != master_period) {
    ...
    for (auto local_iter : sync_status.sync_markers) {
      ...
      ldpp_dout(dpp(), 10) << "SSTRACE: mdlog shard_id = " << shard_id << ", master_marker= " << master_marker << " , local_marker= " << local_iter.second.marker <<dendl;
    ...
    if (ret < 0) {
      ...
      for (auto iter : master_pos) {
        ...
        if (shard_data.entries.empty()) {
          ...
          if (!oldest) {
            ldpp_dout(dpp(), 10) << "SSTRACE: mdlog no oldest found - shard_id = " << iter.first << ", marker= " << shard_data.marker << ", entry.timestamp= " << entry.timestamp << dendl;
            oldest.emplace(iter.first, entry.timestamp);
          } else if (!ceph::real_clock::is_zero(entry.timestamp) && entry.timestamp < oldest->second) {
            ldpp_dout(dpp(), 10) << "SSTRACE: mdlog oldest found - shard_id = " << iter.first << ", marker= " << shard_data.marker << ", entry.timestamp= " << entry.timestamp << ", oldest->second= " <<oldest->second << dendl;
            oldest.emplace(iter.first, entry.timestamp);
          }
        }
      }
                                     ...
                                     int width, std::ostream& out)
{
  ...
  if (r >= 0) {
    ...
    const auto& logs = source_bucket_info.layout.logs;
    if (logs.empty()) {
      out << indented{width} << "init: bucket sync has not started\n";
      return 0;
    }
  ...
  r = rgw_read_remote_bilog_info(dpp, conn, source_bucket,
  ...
  for (const auto& r : remote_markers.get()) {
    ...
    if (shard_id >= total_shards) {
      ...
      ldpp_dout(dpp, 10) << "SSTRACE: bucket log unexpected shard_id = " << shard_id << dendl;
    ...
    ldpp_dout(dpp, 10) << "SSTRACE: bucket log shard_id = " << shard_id << ", local marker position = " << pos << ", remote marker position =" << r.second << dendl;
    if (pos < r.second) {
      shards_behind.insert(shard_id);
    }
  }
...
int main(int argc, const char **argv)
{
  ...
  if (opt_cmd == OPT::BUCKET_RESYNC_ENCRYPTED_MULTIPART) {
    ...
    auto rados = store->getRados();

    // fail if recovery wouldn't generate replication log entries
    if (!rados->svc.zone->need_to_log_data() && !yes_i_really_mean_it) {
    ...
    encode_json("bucket", bucket.name, formatter.get());
    encode_json("bucket_id", bucket.bucket_id, formatter.get());

    ret = rados->bucket_resync_encrypted_multipart(
        dpp(), null_yield, bucket_info, marker, f);
    if (ret < 0) {
      return -ret;
    }
    formatter->close_section();
...
next:
  ...
  if (opt_cmd == OPT::BI_PURGE) {
    ...
    if (ret == -ENOENT) {
      ...
      return -ret;
    } else if (cur_bucket_info.bucket.bucket_id == bucket_info.bucket.bucket_id && !yes_i_really_mean_it) {
    ...
    for (int i = 0; i < max_shards; i++) {
      ...
      int ret = bs.init(dpp(), bucket_info, index, shard_id);
      if (ret < 0) {
        cerr << "ERROR: bs.init(bucket=" << bucket << ", shard=" << i << "): " << cpp_strerror(-ret) << std::endl;
        return -ret;
      }

      ret = static_cast<rgw::sal::RGWRadosStore*>(store)->getRados()->bi_remove(bs);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::OBJECT_REINDEX) {
    ...
    auto process = [&](const std::string& p_object, const std::string& p_object_version) -> int {
      rgw_obj obj(bucket, rgw_obj_key(p_object, p_object_version));
      ret = store->getRados()->reindex_obj(bucket_info, obj, dpp(), null_yield);
      if (ret < 0) {
	return ret;
      }
      return 0;
  ...
  if (opt_cmd == OPT::BUCKET_REWRITE) {
	...
	dpp(), bucket_info, current_index, RGW_NO_SHARD,
  ...
  if (opt_cmd == OPT::BUCKET_RESHARD) {
    ...
    auto zone_svc = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->zone;
    ...
    RGWBucketReshard br(static_cast<rgw::sal::RGWRadosStore*>(store),
			bucket_info, attrs,
			nullptr /* no callback */);
...
#define MAX_LC_LIST_ENTRIES 100
  ...
  if (opt_cmd == OPT::BUCKET_SYNC_INIT) {
    ...
    auto sync = RGWBucketPipeSyncStatusManager::construct(
      dpp(), static_cast<rgw::sal::RGWRadosStore*>(store), source_zone, opt_sb,
      bucket, extra_info ? &std::cout : nullptr);
  ...
  if (opt_cmd == OPT::BUCKET_SYNC_MARKERS) {
    ...
    auto sync = RGWBucketPipeSyncStatusManager::construct(
      dpp(), static_cast<rgw::sal::RGWRadosStore*>(store), source_zone,
      opt_source_bucket, bucket, nullptr);
  ...
  if (opt_cmd == OPT::BUCKET_SYNC_RUN) {
    ...
    auto sync = RGWBucketPipeSyncStatusManager::construct(
      dpp(), static_cast<rgw::sal::RGWRadosStore*>(store), source_zone,
      opt_source_bucket, bucket, extra_info ? &std::cout : nullptr);
  ...
  if (opt_cmd == OPT::BILOG_LIST) {
    ...
    do {
      ...
      ret = store->svc()->bilog_rados->log_list(dpp(), bucket_info, log_layout, shard_id, marker, max_entries - count, entries, &truncated);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT::BILOG_TRIM) {
    ...
    ret = bilog_trim(dpp(), static_cast<rgw::sal::RGWRadosStore*>(store),
		     bucket_info, *gen,
  ...
  if (opt_cmd == OPT::BILOG_STATUS) {
    ...
    ret = static_cast<rgw::sal::RGWRadosStore*>(store)->svc()->bilog_rados->get_log_status(dpp(), bucket_info, log_layout, shard_id,
						    &markers, null_yield);
    if (ret < 0) {
