                                     ...
                                     int width, std::ostream& out)
{
  ...
  if (r < 0) {
    ldpp_dout(dpp, -1) << "failed to read bucket incremental sync status: " << cpp_strerror(r) << dendl;
    return r;
  }
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
  ...
  if (opt_cmd == OPT::RESHARD_CANCEL) {
	...
	// we cancelled unerway resharding above, so we're good
	return 0;
      }
    } else if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::BUCKET_SYNC_RUN) {
  ...
  if (opt_cmd == OPT::BILOG_STATUS) {
    ...
    ret = store->svc()->bilog_rados->get_log_status(dpp(), bucket_info, log_layout, shard_id,
						    &markers, null_yield);
    if (ret < 0) {
