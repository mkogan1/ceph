                                     ...
                                     int width, std::ostream& out)
{
  ...
  int r = init_bucket(pipe.source.bucket->tenant,
		      pipe.source.bucket->name,
		      pipe.source.bucket->bucket_id,
		      source_bucket_info,
		      source_bucket);
  if (r < 0) {
  ...
  if (r >= 0) {
    ...
    const auto& log = source_bucket_info.layout.logs.front();
    if (log.gen > 0) {
  ...
  r = rgw_read_bucket_inc_sync_status(dpp(), store, pipe, full_status.incremental_gen,  &shard_status);
  if (r < 0) {
			       ...
			       RGWBucketInfo& new_bucket_info)
{
  ...
  int ret = store->init_bucket_index(new_bucket_info,
				     new_bucket_info.layout.current_index);
  if (ret < 0) {
...
static int trim_sync_error_log(int shard_id, const string& marker, int delay_ms)
{
  ...
  for (;;) {
    int ret = store->time_log_trim(oid, {}, {}, {}, marker);
    if (ret == -ENODATA) {
      return 0;
    }
    if (ret < 0) {
...
next:
  ...
  if (opt_cmd == OPT_BI_LIST) {
    ...
    for (; i < max_shards; i++) {
      ...
      int ret = bs.init(bucket_info, index, i);
      marker.clear();
  ...
  if (opt_cmd == OPT_BUCKET_RESHARD) {
    ...
    RGWBucketReshard br(store, bucket_info, bucket_attrs, nullptr /* no callback */);
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
  ...
  if (opt_cmd == OPT_RESHARD_STATUS) {
    ...
    ret = init_bucket(tenant, bucket_name, bucket_id, bucket_info, bucket, &bucket_attrs);
    if (ret < 0) {
      cerr << "ERROR: could not init bucket: " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
  ...
  if (opt_cmd == OPT_GC_LIST) {
    ...
    do {
      ...
      int ret = store->list_gc_objs(&index, marker, 1000, !include_all, result, &truncated, processing_queue);
      if (ret < 0) {
...
#define MAX_LC_LIST_ENTRIES 100
    ...
    do {
      int ret =
	store->list_lc_progress(marker, max_entries, bucket_lc_map, index);
      if (ret < 0) {
  ...
  if (opt_cmd == OPT_LC_PROCESS) {
    ...
    int ret = store->process_lc(optional_bucket);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT_METADATA_PUT) {
    ...
    ret = store->meta_mgr->put(metadata_key, bl, RGWMetadataHandler::RGWMetadataHandler::APPLY_ALWAYS, false);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT_BUCKET_SYNC_STATUS) {
    ...
    bucket_sync_status(store, bucket_info, source_zone, std::nullopt, std::cout);
  }
 ...
 if (opt_cmd == OPT_BUCKET_SYNC_RUN) {
  ...
  if (opt_cmd == OPT_SYNC_ERROR_LIST) {
    ...
    for (; shard_id < ERROR_LOGGER_SHARDS; ++shard_id) {
      ...
      do {
        list<cls_log_entry> entries;
        ret = store->time_log_list(oid, {}, {}, max_entries - count, entries,
  ...
  if (opt_cmd == OPT_BILOG_TRIM) {
    ...
    ret = bilog_trim(store, bucket_info, *gen,
