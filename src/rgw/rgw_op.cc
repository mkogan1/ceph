...
static int get_obj_attrs(RGWRados *store, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& obj, map<string, bufferlist>& attrs)
{
  RGWRados::Object op_target(store, bucket_info, obj_ctx, obj);
...
static int modify_obj_attr(RGWRados *store, RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& obj, const char* attr_name, bufferlist& attr_val)
{
  map<string, bufferlist> attrs;
  RGWRados::Object op_target(store, bucket_info, obj_ctx, obj);
  RGWRados::Object::Read read_op(&op_target);
...
static int rgw_iam_add_existing_objtags(RGWRados* store, struct req_state* s, rgw_obj& obj, std::uint64_t action){
  map <string, bufferlist> attrs;
  store->set_atomic(s->obj_ctx, obj);
  int op_ret = get_obj_attrs(store, *static_cast<RGWObjectCtx *>(s->obj_ctx), s->bucket_info, obj, attrs);
  if (op_ret < 0)
    return op_ret;
...
void RGWGetObjTags::execute()
{
  ...
  op_ret = get_obj_attrs(store, *static_cast<RGWObjectCtx *>(s->obj_ctx), s->bucket_info, obj, attrs);
  if (op_ret < 0) {
...
void RGWPutObjTags::execute()
{
  ...
  obj = rgw_obj(s->bucket, s->object);
  store->set_atomic(s->obj_ctx, obj);
  op_ret = modify_obj_attr(store, *static_cast<RGWObjectCtx *>(s->obj_ctx), s->bucket_info, obj, RGW_ATTR_TAGS, tags_bl);
  if (op_ret == -ECANCELED){
...
int RGWListBuckets::verify_permission()
{
  ...
  if (!verify_user_permission(s, ARN(partition, service, "", s->user->user_id.tenant, "*"), rgw::IAM::s3ListAllMyBuckets)) {
    return -EACCES;
  }
...
int RGWListBucket::parse_max_keys()
{
  ...
  return parse_value_and_bound(max_keys, // input string
			       max, // output int
			       0, // lower-bound
			       s->cct->_conf->get_val<uint64_t>("rgw_max_listing_results"), // upper-bound
...
int RGWCreateBucket::verify_permission()
{
  ...
  ARN arn = ARN(bucket);
  if (!verify_user_permission(s, arn, rgw::IAM::s3CreateBucket)) {
    return -EACCES;
  }
...
void RGWPutMetadataObject::execute()
{
  ...
  op_ret = get_obj_attrs(store, *static_cast<RGWObjectCtx *>(s->obj_ctx), s->bucket_info, obj, orig_attrs);
  if (op_ret < 0) {
    return;
  }
...
#define ACL_GRANTS_MAX_NUM      100
  ...
  if (!s->object.empty()) {
    ...
    op_ret = modify_obj_attr(store, *static_cast<RGWObjectCtx *>(s->obj_ctx), s->bucket_info, obj, RGW_ATTR_ACL, bl);
  } else {
