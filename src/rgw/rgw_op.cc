...
void RGWDeleteObj::execute(optional_yield y)
{
  ...
  if (!rgw::sal::RGWObject::empty(s->object.get())) {
    ...
    // send request to notification manager
    const auto ret = rgw::notify::publish_commit(s->object.get(), obj_state->size, obj_state->mtime, attrs[RGW_ATTR_ETAG].to_str(), version_id, event_type, res, this);
    if (ret < 0) {
      ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
      // too late to rollback operation, hence op_ret is not set here
    }
...
void RGWCopyObj::execute(optional_yield y)
{
  ...
  // send request to notification manager
  const auto ret = rgw::notify::publish_commit(s->object.get(), astate->size, mtime, etag, dest_object->get_instance(), event_type, res, this);
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
    // too late to rollback operation, hence op_ret is not set here
  }
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
  ...
  for (iter = multi_delete->objects.begin();
    ...
    // send request to notification manager
    const auto ret = rgw::notify::publish_commit(obj.get(), obj_state->size, obj_state->mtime, etag, version_id, event_type, res, this);
    if (ret < 0) {
      ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
      // too late to rollback operation, hence op_ret is not set here
    }
