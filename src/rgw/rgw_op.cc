...
void RGWPutObj::execute(optional_yield y)
{
  ...
  if (rdma_put) {
    ...
    if (total == 0) {
      ldpp_dout(this, 0) << "rgw_cuobj: failed to parse size from RDMA descriptor" << dendl;
      op_ret = -EINVAL;
      return;
    }
    ldpp_dout(this, 10) << "rgw_cuobj: RDMA PUT size=" << total
                        << " from descriptor" << dendl;
    auto* rbuf = cuobj_srv->acquire_buffer(total);
    if (!rbuf) {
      ldpp_dout(this, 0) << "rgw_cuobj: no RDMA buffer available for "
                         << total << " bytes" << dendl;
      op_ret = -ERR_SERVICE_UNAVAILABLE;
      return;
    }

    ssize_t rdma_ret = cuobj_srv->rdma_read_from_client(
        s->object->get_name(), rbuf, 0, total, rdma_descr);
    if (rdma_ret < 0) {
      cuobj_srv->release_buffer(rbuf);
      ldpp_dout(this, 0) << "rgw_cuobj: RDMA read failed: "
                         << rdma_ret << dendl;
      op_ret = -EIO;
      return;
    }
    ...
    while (data_ofs < rdma_total) {
      ...
      if (need_calc_md5) {
        hash.Update(reinterpret_cast<const unsigned char*>(data.c_str()),
                    data.length());
      }
