                                       ...
                                       bool swift_slo)
{
  ...
  if (s->system_request) {
    ...
    ldpp_dout(this, 2) << "overriding permissions due to admin operation" << dendl;
				       ...
				       &obj_policy, bucket_policy, s->iam_user_policies, s->session_policies, action)) {
    return -EPERM;
  }
  if (ent.meta.size == 0) {
    return 0;
...
void RGWDeleteBucket::execute()
{
  ...
  RGWBucketEnt ent;
  op_ret = rgw_bucket_sync_user_stats(store, s->user->user_id, s->bucket_info, &ent);
  if ( op_ret < 0) {
...
int RGWPutObj::verify_permission()
{
  ...
  if (s->iam_policy || ! s->iam_user_policies.empty() || !s->session_policies.empty()) {
    ...
    if (s->iam_policy) {
      ARN obj_arn(rgw_obj(s->bucket, s->object));
...
void RGWPutObj::execute()
{
  ...
  const auto ret = rgw::notify::publish_commit(obj.key, s->obj_size, mtime, etag, event_type, res);
  if (ret < 0) {
...
void RGWPostObj::execute()
{
  ...
  const auto ret = rgw::notify::publish_commit(s->object, ofs, ceph::real_clock::now(), etag, event_type, res);
  if (ret < 0) {
		ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
		// too late to rollback operation, hence op_ret is not set here
  }
...
int RGWDeleteObj::verify_permission()
{
  ...
  if (s->iam_policy || ! s->iam_user_policies.empty() || ! s->session_policies.empty()) {
    ...
    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    ARN obj_arn(ARN(s->bucket, s->object.name));
    if (s->iam_policy) {
...
void RGWDeleteObj::execute()
{
  ...
  if (!s->object.empty()) {
    ...
    const auto ret = rgw::notify::publish_commit(s->object, obj_state->size , obj_state->mtime, attrs[RGW_ATTR_ETAG].to_str(),
        event_type, res);
...
int RGWCopyObj::verify_permission()
{
  ...
  if (src_bucket_name.compare(dest_bucket_name) == 0) { /* will only happen if s->local_source
    ...
    if (op_ret < 0) {
      if (op_ret == -ENOENT) {
        ldpp_dout(this, 0) << "ERROR: Bucket not found for user: " << s->user->user_id.to_str() << dendl;
	op_ret = -ERR_NO_SUCH_BUCKET;
      }
      return op_ret;
    }
...
void RGWCopyObj::execute()
{
  ...
  const auto ret = rgw::notify::publish_commit(s->object, s->obj_size, mtime, etag, event_type, res);;
  if (ret < 0) {
		ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
		// too late to rollback operation, hence op_ret is not set here
  }
...
void RGWInitMultipart::execute()
{
  ...
  const auto ret = rgw::notify::publish_commit(s->object, s->obj_size, ceph::real_clock::now(), attrs[RGW_ATTR_ETAG].to_str(), event_type, res);
  if (ret < 0) {
		ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
		// too late to rollback operation, hence op_ret is not set here
  }
...
int RGWCompleteMultipart::verify_permission()
{
  ...
  if (s->iam_policy || ! s->iam_user_policies.empty() || ! s->session_policies.empty()) {
    ...
    rgw::IAM::Effect e = Effect::Pass;
    rgw::IAM::PolicyPrincipal princ_type = rgw::IAM::PolicyPrincipal::Other;
    rgw::ARN obj_arn(rgw_obj(s->bucket, s->object));
    if (s->iam_policy) {
      e = s->iam_policy->eval(s->env, *s->auth.identity,
				 rgw::IAM::s3PutObject,
				 obj_arn,
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
  ...
  for (iter = multi_delete->objects.begin();
    ...
    const auto ret = rgw::notify::publish_commit(obj.key, obj_state->size, obj_state->mtime, etag, event_type, res);
    if (ret < 0) {
			ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
			// too late to rollback operation, hence op_ret is not set here
    }
