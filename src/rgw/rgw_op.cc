  ...
  if (ret == -ENOENT) {
    ...
    if (bucket_owner.compare(s->user->get_id()) != 0 &&
      ...
      if (policy) {
        ...
        auto r =  policy->eval(s->env, *s->auth.identity, rgw::IAM::s3ListBucket, b_arn);
        if (r == Effect::Allow)
          return -ENOENT;
        if (r == Effect::Deny)
          return -EACCES;
...
void RGWDeleteObj::execute(optional_yield y)
{
  ...
  if (!rgw::sal::RGWObject::empty(s->object.get())) {
    ...
    RGWObjectCtx* obj_ctx = static_cast<RGWObjectCtx *>(s->obj_ctx);
    {
      ...
      op_ret = s->object->get_obj_state(obj_ctx, *(s->bucket.get()), &astate, s->yield, true);
      if (op_ret < 0) {
...
void RGWCompleteMultipart::execute(optional_yield y)
{
  ...
  int r = meta_obj->delete_object(s->obj_ctx, ACLOwner(), ACLOwner(), ceph::real_time(), false, 0,
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
  ...
  for (iter = multi_delete->objects.begin();
    ...
    if (!rgw::sal::RGWObject::empty(obj.get())) {
      ...
      const auto ret = obj->get_obj_state(obj_ctx, *bucket,  &astate, s->yield, true);
...
bool RGWBulkDelete::Deleter::delete_single(const acct_path_t& path, optional_yield y)
{
  ...
  if (!path.obj_key.empty()) {
    ...
    ret = obj->delete_object(s->obj_ctx, bowner, bucket_owner, ceph::real_time(), false, 0,
			     version_id, s->yield);
    if (ret < 0) {
      goto delop_fail;
    }
