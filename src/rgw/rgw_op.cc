...
void RGWPutObj::execute(optional_yield y)
{
  ...
  if (!multipart) {
    op_ret = rgw::bucketlogging::log_record(driver, rgw::bucketlogging::LoggingType::Journal, s->object.get(), s, canonical_name(), etag, s->object->get_obj_size(), this, y, false, false);
    if (op_ret  < 0) {
      return;
   }
  }
...
void RGWDeleteMultiObj::handle_individual_object(const RGWMultiDelObject& object, optional_yield y, boost::asio::deadline_timer *formatter_flush_cond)
{
