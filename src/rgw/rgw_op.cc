...
static int rgw_iam_remove_objtags(const DoutPrefixProvider *dpp, struct req_state* s, rgw::sal::RGWObject* object, bool has_existing_obj_tag, bool has_resource_tag) {
  object->set_atomic(s->obj_ctx);
  int op_ret = object->get_obj_attrs(s->obj_ctx, s->yield, dpp);
  if (op_ret < 0)
    return op_ret;
  rgw::sal::RGWAttrs attrs = object->get_attrs();
...
static int rgw_iam_add_objtags(const DoutPrefixProvider *dpp, struct req_state* s, rgw::sal::RGWObject* object, bool has_existing_obj_tag, bool has_resource_tag) {
  object->set_atomic(s->obj_ctx);
  int op_ret = object->get_obj_attrs(s->obj_ctx, s->yield, dpp);
  if (op_ret < 0)
    return op_ret;
...
static int rgw_iam_add_buckettags(const DoutPrefixProvider *dpp, struct req_state* s, rgw::sal::RGWBucket* bucket) {
...
void RGWPutObj::execute(optional_yield y)
{
  ...
  const auto ret = rgw::notify::publish_commit(s->object.get(), s->obj_size, mtime, etag, s->object->get_instance(), event_type, res, this);
  if (ret < 0) {
...
void RGWPostObj::execute(optional_yield y)
{
  ...
  // send request to notification manager
  const auto ret = rgw::notify::publish_commit(s->object.get(), ofs, s->object->get_mtime(), etag, s->object->get_instance(), event_type, res, this);
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
    // too late to rollback operation, hence op_ret is not set here
  }
...
void RGWDeleteObj::execute(optional_yield y)
{
  ...
  if (!rgw::sal::RGWObject::empty(s->object.get())) {
    ...
    // send request to notification manager
    const auto ret = rgw::notify::publish_commit(s->object.get(), obj_size, ceph::real_clock::now(), attrs[RGW_ATTR_ETAG].to_str(), version_id, event_type, res, this);
    if (ret < 0) {
      ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
      // too late to rollback operation, hence op_ret is not set here
    }
...
void RGWCopyObj::execute(optional_yield y)
{
  ...
  // send request to notification manager
  const auto ret = rgw::notify::publish_commit(s->object.get(), obj_size, mtime, etag, dest_object->get_instance(), event_type, res, this);
  if (ret < 0) {
    ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
    // too late to rollback operation, hence op_ret is not set here
  }
...
#define DELETE_MULTI_OBJ_MAX_NUM      1000
  ...
  for (iter = multi_delete->objects.begin();
    ...
    // send request to notification manager
    const auto ret = rgw::notify::publish_commit(obj.get(), obj_size, ceph::real_clock::now(), etag, version_id, event_type, res, this);
    if (ret < 0) {
      ldpp_dout(this, 1) << "ERROR: publishing notification failed, with error: " << ret << dendl;
      // too late to rollback operation, hence op_ret is not set here
    }
