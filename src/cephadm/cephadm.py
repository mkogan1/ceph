...
DEFAULT_NVMEOF_IMAGE = 'registry.redhat.io/rhceph/ceph-nvmeof-rhel9:1.2'
...
class CephNvmeof(object):
    ...
    def get_container_mounts(data_dir: str, log_dir: str, mtls_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        mounts[os.path.join(data_dir, 'config')] = '/etc/ceph/ceph.conf:z'
        mounts[os.path.join(data_dir, 'keyring')] = '/etc/ceph/keyring:z'
        mounts[os.path.join(data_dir, 'ceph-nvmeof.conf')] = '/remote-source/ceph-nvmeof/app/ceph-nvmeof.conf:z'
        mounts[os.path.join(data_dir, 'configfs')] = '/sys/kernel/config'
...
def get_container_mounts(ctx, fsid, daemon_type, daemon_id,
    ...
    if daemon_type == CephNvmeof.daemon_type:
        ...
        mtls_dir = os.path.join(ctx.data_dir, fsid, 'mtls')
